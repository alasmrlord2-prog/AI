...
class PolicyService:
    ...
    def get_all_relations(
        ...
        if subject_type:
            query = query.filter(RelationTuple.subject_type == subject_type)
        if subject_id:
            query = query.filter(RelationTuple.subject_id == subject_id)
        if object_type:
            query = query.filter(RelationTuple.object_type == object_type)
        if object_id:
            query = query.filter(RelationTuple.object_id == object_id)
        if tenant_id:
            query = query.filter(
                or_(
                    RelationTuple.tenant_id == tenant_id,
        ...
        if tenant_id:
            query = query.filter(
                or_(
                    ...
                    PolicyRule.tenant_id.is_(None)
                )
            )
        return query.order_by(PolicyRule.priority.desc()).all()
