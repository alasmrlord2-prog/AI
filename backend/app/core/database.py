...
def set_sqlite_pragma(dbapi_conn, connection_record):
    ...
    # Check if SQLite by examining the connection
    try:
        # Try to get URL from connection record
        url = getattr(connection_record, 'info', {}).get('url', '')
        if not url and hasattr(connection_record, 'engine'):
