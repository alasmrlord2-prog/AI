...
def api_read_file(
    ...
    if action_id:
        ...
        if action and action.get("status") == "approved" and action.get("execution_result") is not None:
            # Return the execution result from the approved action
            return {
                "content": action.get("execution_result"),
                "from_approved_action": True,
                "action_id": action_id
            }
...
async def api_run_shell(
    payload: ToolRunShell, 
    current_user: dict = Depends(get_current_user),
    action_id: str = Query(None, description="Optional: Get result from approved action")
):
    ...
    if action_id:
        ...
        if action and action.get("status") == "approved" and action.get("execution_result") is not None:
            # Return the execution result from the approved action
            return {
                "output": action.get("execution_result"),
                "from_approved_action": True,
                "action_id": action_id
            }
        elif action and action.get("status") == "pending":
...
def api_service(
    ...
    if action_id:
        ...
        if action and action.get("status") == "approved" and action.get("execution_result") is not None:
            # Return the execution result from the approved action
            return {
                "status": action.get("execution_result"),
                "from_approved_action": True,
                "action_id": action_id
            }
        elif action and action.get("status") == "pending":
