...
const ORIGIN_NAVIGATION_COMMAND = 'cursor.browserOriginAllowlist.ensureNavigationAllowed';
const ORIGIN_PAGE_COMMAND = 'cursor.browserOriginAllowlist.ensurePageOriginAllowed';
const BROWSER_GET_URL_COMMAND = 'cursor.browserView.getURL';

async function ensureBrowserNavigationAllowed(url: string): Promise<void> {
	await vscode.commands.executeCommand(ORIGIN_NAVIGATION_COMMAND, { url });
}

async function ensureCurrentPageAllowed(toolName: string, viewId: string): Promise<void> {
	const currentUrl = await vscode.commands.executeCommand<string | undefined>(BROWSER_GET_URL_COMMAND, viewId);
	await vscode.commands.executeCommand(ORIGIN_PAGE_COMMAND, { toolName, url: currentUrl });
}
...
async function executeInBrowser(code: string, viewId: string): Promise<unknown> {
	try {
		return await vscode.commands.executeCommand('cursor.browserView.executeJavaScript', code, viewId);
	} catch (error) {
...
interface FormatPageStateOptions {
	/** The action name (e.g., 'click', 'type', 'hover') */
	action: string;
	/** Human-readable detail lines to include in the output */
	detailLines?: string[];
	/** The page state containing url, title, and snapshot */
	pageState: PageState;
}

/**
 * Format a browser action result with properly formatted YAML snapshot
 */
async function formatPageStateResult(options: FormatPageStateOptions): Promise<McpToolResult> {
	const { action, detailLines, pageState } = options;

	if (pageState?.snapshot) {
		// Convert the accessibility tree object to YAML format string
		const ariaSnapshot = accessibilityTreeToYaml(pageState.snapshot);
		...
		if (snapshotSize > SNAPSHOT_SIZE_THRESHOLD) {
			try {
				...
				textParts.push(`### Action: ${action}`);
				if (detailLines && detailLines.length > 0) {
					textParts.push(...detailLines);
				}
				textParts.push(
					`\n### Page state`,
					`- Page URL: ${url}`,
	...
	// If we reach this point, for some reason there's no snapshot in the pageState
	// so just return whatever is in the result
	return {
...
export const BrowserTools = {
	async navigate(params: { url: string; viewId: string; position?: 'active' | 'side' }): Promise<McpToolResult> {
		await ensureBrowserNavigationAllowed(params.url);
		CursorIDEBrowserLogger.info(`Navigating to ${params.url}`);

		const listResult = await vscode.commands.executeCommand<{ tabs: string[] }>('cursor.browserView.listTabs');
		const tabs = listResult?.tabs ?? [];

	if (params.viewId || tabs.length > 0) {
		const targetViewId = params.viewId || tabs[0];
		await vscode.commands.executeCommand('cursor.browserView.navigate', params.url, targetViewId);
		await new Promise(resolve => setTimeout(resolve, 1000));
		return await BrowserTools.snapshot({ viewId: targetViewId });
	}

	const options = params.position ? { position: params.position } : undefined;
	const result = await vscode.commands.executeCommand<{ success: boolean; browserId: string }>('cursor.browserView.newTab', params.url, options);
	if (!result?.browserId) {
		return {
			content: [{ type: 'text', text: 'Failed to create new browser tab' }]
		};
	}
	await new Promise(resolve => setTimeout(resolve, 1000));
	return await BrowserTools.snapshot({ viewId: result.browserId });
	},

	async snapshot(params: { viewId: string }): Promise<McpToolResult> {
		await ensureCurrentPageAllowed('browser_snapshot', params.viewId);
		const code = `
			${BROWSER_UTILS}
			(function() {
		...
		`;
		const result = await executeInBrowser(code, params.viewId) as {
			success: boolean;
			pageState: PageState;
		};

		return await formatPageStateResult({
			action: 'snapshot',
			pageState: result.pageState,
		});
	},

	async click(params: { ref: string; element?: string; doubleClick?: boolean; button?: string; modifiers?: string[]; viewId: string }): Promise<McpToolResult> {
		await ensureCurrentPageAllowed('browser_click', params.viewId);
		const code = `
			${BROWSER_UTILS}
			(function() {
		...
		detailLines.push(`- Click type: ${result.doubleClick ? 'double-click' : 'single-click'}`);
		detailLines.push(`- Button: ${result.button}`);
	...
	async type(params: { ref: string; text: string; element?: string; submit?: boolean; slowly?: boolean; viewId: string }): Promise<McpToolResult> {
		await ensureCurrentPageAllowed('browser_type', params.viewId);
		const code = `
			${BROWSER_UTILS}
			(function() {
				const ref = ${JSON.stringify(params.ref)};
		...
		const detailLines: string[] = [];
		detailLines.push(`- Characters typed: ${result.textLength}`);
		detailLines.push(`- Typing mode: ${result.slowly ? 'slow (character-by-character)' : 'fast'}`);
		if (result.submitted) {
			detailLines.push(`- Form submitted: yes (Enter key pressed)`);
		}
	...
	async hover(params: { ref: string; element?: string; viewId: string }): Promise<McpToolResult> {
		await ensureCurrentPageAllowed('browser_hover', params.viewId);
		const code = `
			${BROWSER_UTILS}
			(function() {
				const ref = ${JSON.stringify(params.ref)};
	...
	async selectOption(params: { ref: string; values: string[]; element?: string; viewId: string }): Promise<McpToolResult> {
		await ensureCurrentPageAllowed('browser_select_option', params.viewId);
		const code = `
			${BROWSER_UTILS}
			(function() {
				const ref = ${JSON.stringify(params.ref)};
		...
		if (result.selectedValues.length > 0) {
			detailLines.push(`- Selected values: ${result.selectedValues.join(', ')}`);
		} else {
			detailLines.push(`- Selected values: (none)`);
		}
	...
	async pressKey(params: { key: string; viewId: string }): Promise<McpToolResult> {
		await ensureCurrentPageAllowed('browser_press_key', params.viewId);
		const code = `
			(function() {
		...
		`;
		const result = await executeInBrowser(code, params.viewId) as { action: string; success: boolean; key: string };
		if (!result.success) {
			return {
				content: [{ type: 'text', text: `Failed to press key: ${result.key}` }]
			};
		}
		return {
			content: [{ type: 'text', text: `Successfully pressed key: ${result.key}` }]
		};
	},

	async waitFor(params: { time?: number; text?: string; textGone?: string; viewId: string }): Promise<McpToolResult> {
		await ensureCurrentPageAllowed('browser_wait_for', params.viewId);
		if (params.time !== undefined) {
			await new Promise(resolve => setTimeout(resolve, params.time! * 1000));
			return {
				content: [{ type: 'text', text: `Successfully waited for: ${params.time} seconds` }]
			};
		}
		...
		`;
		const { success, type, text, error } = await executeInBrowser(code, params.viewId) as { action: string; success: boolean; type: string; text?: string; error?: string };
		if (!success) {
			return {
				content: [{ type: 'text', text: `Failed to wait for element: ${error || 'Unknown error'}` }]
			};
		}
		return {
			content: [{ type: 'text', text: `Successfully waited for element: ${text || 'Unknown text'}` }]
		};
	},

	async consoleMessages(params: { viewId: string }): Promise<McpToolResult> {
		await ensureCurrentPageAllowed('browser_console_messages', params.viewId);
		const logs = await vscode.commands.executeCommand<Array<{ type: string; message: string; timestamp: number }>>('cursor.browserView.getConsoleLogs', params.viewId);
		return {
			content: [{ type: 'text', text: JSON.stringify(logs, null, 2) }]
		};
	},

	async networkRequests(params: { viewId: string }): Promise<McpToolResult> {
		await ensureCurrentPageAllowed('browser_network_requests', params.viewId);
		const requests = await vscode.commands.executeCommand<Array<{
			...
			timestamp: number;
			resourceType?: string;
		}>>('cursor.browserView.getNetworkRequests', params.viewId);
		return {
			content: [{ type: 'text', text: JSON.stringify(requests, null, 2) }]
		};
	},

	async takeScreenshot(params: { filename?: string; type?: string; fullPage?: boolean; element?: string; ref?: string; viewId: string }): Promise<McpToolResult> {
		await ensureCurrentPageAllowed('browser_take_screenshot', params.viewId);

		// Ensure filename has the correct extension if provided without one
		let processedParams = params;
		if (params.filename) {
			const hasExtension = /\.[a-zA-Z0-9]+$/.test(params.filename);
			if (!hasExtension) {
				const fileType = params.type || 'png';
				processedParams = { ...params, filename: `${params.filename}.${fileType}` };
			}
		}

		// Screenshot will be handled by the main process
		const screenshotResult = await vscode.commands.executeCommand('cursor.browserView.takeScreenshot', processedParams) as {
			success: boolean;
			filename?: string;
			savedPath?: string;
			dataUrl?: string;
	...
	async goBack(params: { viewId: string }): Promise<McpToolResult> {
		await ensureCurrentPageAllowed('browser_navigate_back', params.viewId);
		const success = await vscode.commands.executeCommand('cursor.browserView.goBack', params.viewId) as boolean;
		if (!success) {
			return {
				content: [{ type: 'text', text: 'Failed to navigate back' }]
			};
		}
		return {
			content: [{ type: 'text', text: 'Successfully navigated back' }]
		};
	},

	async resize(params: { width: number; height: number; viewId: string }): Promise<McpToolResult> {
		await ensureCurrentPageAllowed('browser_resize', params.viewId);
		const { success } = await vscode.commands.executeCommand('cursor.browserView.resize', { ...params, viewId: params.viewId }) as { success: boolean };
		if (!success) {
			return {
				content: [{ type: 'text', text: 'Failed to resize browser' }]
			};
		}
		return {
			content: [{ type: 'text', text: 'Successfully resized browser' }]
		};
	},

	async tabs(params: { action: string; index?: number; position?: 'active' | 'side' }): Promise<McpToolResult> {
		switch (params.action) {
			case 'list': {
				const result = await vscode.commands.executeCommand<{ tabs: string[]; activeTab?: string; lastInteractedTab?: string }>('cursor.browserView.listTabs');
				if (!result || !result.tabs) {
					return { content: [{ type: 'text', text: 'Open tabs:\n(none)' }] };
				}
				const tabInfo = await Promise.all(result.tabs.map(async (tabId: string, idx: number) => {
					const url = await vscode.commands.executeCommand<string>('cursor.browserView.getURL', tabId);
					return `[${idx}] (viewId: ${tabId}) ${url || 'about:blank'}`;
				}));
				return { content: [{ type: 'text', text: `Open tabs:\n${tabInfo.join('\n')}` }] };
			}
			case 'new': {
				const options = params.position ? { position: params.position } : undefined;
				await vscode.commands.executeCommand<{ success: boolean; browserId: string }>('cursor.browserView.newTab', undefined, options);
				return { content: [{ type: 'text', text: `Created new tab${params.position === 'side' ? ' on the side' : ''}` }] };
			}
			case 'close': {
				const listResult = await vscode.commands.executeCommand<{ tabs: string[] }>('cursor.browserView.listTabs');
				const tabs = listResult?.tabs ?? [];
				const targetId = params.index !== undefined ? tabs[params.index] : undefined;
				await vscode.commands.executeCommand('cursor.browserView.closeTab', targetId);
				const afterCloseResult = await vscode.commands.executeCommand<{ tabs: string[]; activeTab?: string; lastInteractedTab?: string }>('cursor.browserView.listTabs');
				const activeViewId = afterCloseResult?.activeTab ?? afterCloseResult?.lastInteractedTab;
				if (!activeViewId) {
					return { content: [{ type: 'text', text: 'Tab closed. No remaining tabs.' }] };
				}
				return await BrowserTools.snapshot({ viewId: activeViewId });
			}
			case 'select': {
				...
				const listResult = await vscode.commands.executeCommand<{ tabs: string[] }>('cursor.browserView.listTabs');
				const tabs = listResult?.tabs ?? [];
				const targetId = tabs[params.index];
				if (!targetId) {
					throw new Error(`Tab ${params.index} not found`);
				}
				await vscode.commands.executeCommand('cursor.browserView.selectTab', targetId);
				return await BrowserTools.snapshot({ viewId: targetId });
			}
			default:
