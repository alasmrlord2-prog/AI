...
async def monitor_status():
    ...
    # Get base monitoring data with reasonable timeout (max 8 seconds)
    try:
        loop = asyncio.get_event_loop()
        monitor_result = await asyncio.wait_for(
            loop.run_in_executor(None, monitor_run),
            timeout=8.0  # Increased to 8 seconds to allow proper execution
        )
    ...
    # Get network monitoring with reasonable timeout (max 5 seconds)
    try:
        loop = asyncio.get_event_loop()
        network_data = await asyncio.wait_for(
            loop.run_in_executor(None, network_monitor),
            timeout=5.0  # Increased to 5 seconds to allow proper execution
        )
        base_data["network"] = network_data
    except asyncio.TimeoutError:
        print("Network monitor timeout - using default")
        base_data["network"] = {"error": "timeout"}
    except Exception as e:
