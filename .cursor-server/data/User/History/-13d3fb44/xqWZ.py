...
# Simple in-memory cache for monitor data (TTL: 2 seconds)
...
async def monitor_status():
    ...
    # Get base monitoring data with reasonable timeout (max 5 seconds) - OPTIMIZED
    try:
        loop = asyncio.get_event_loop()
        monitor_result = await asyncio.wait_for(
            loop.run_in_executor(None, monitor_run),
            timeout=5.0  # Reduced to 5 seconds - monitor_run is now faster
        )
    ...
    # Get network monitoring with reasonable timeout (max 3 seconds) - OPTIMIZED
    try:
        loop = asyncio.get_event_loop()
