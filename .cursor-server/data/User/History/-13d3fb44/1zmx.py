...
async def monitor_status():
    ...
    # Get base monitoring data with aggressive timeout (max 2 seconds)
    try:
    ...
    # Add network monitoring with very aggressive timeout (max 1 second)
    try:
        loop = asyncio.get_event_loop()
        network_data = await asyncio.wait_for(
            loop.run_in_executor(None, network_monitor),
            timeout=1.0  # Reduced to 1 second - network monitoring is optional
        )
