...
async def monitor_status():
    ...
    # Get base monitoring data with timeout protection (max 5 seconds)
    try:
        loop = asyncio.get_event_loop()
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(monitor_run)
            base_data = await asyncio.wait_for(
    ...
    # Add network monitoring with timeout protection (max 3 seconds)
    try:
        loop = asyncio.get_event_loop()
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(network_monitor)
