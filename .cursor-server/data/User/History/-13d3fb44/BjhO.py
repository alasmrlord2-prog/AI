...
def monitor_status():
    ...
    if isinstance(base_data.get("network"), dict):
        ...
        base_data["network_rx_mbps"] = network.get("rx_mbps") or network.get("stats", {}).get("total", {}).get("rx_mbps") or network.get("rx") or 0
        base_data["network_tx_mbps"] = network.get("tx_mbps") or network.get("stats", {}).get("total", {}).get("tx_mbps") or network.get("tx") or 0
