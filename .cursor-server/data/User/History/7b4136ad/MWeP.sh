...
# Start required services first (postgres, ollama)
echo "📦 Starting required services (postgres, ollama)..."
docker-compose -f "$COMPOSE_FILE" up -d postgres ollama

# Wait for postgres and ollama to be ready
echo ""
echo "⏳ Waiting for postgres and ollama to start..."
sleep 5
...
if docker ps | grep -q "ai-agent-backend-prod"; then
    ...
    echo "   Docs: http://$DOMAIN/api/docs"
