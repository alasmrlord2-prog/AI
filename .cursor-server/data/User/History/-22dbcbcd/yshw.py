...
async def get_services_list(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        response = requests.get("http://localhost:8000/api/capabilities", timeout=3)
        ...
        for category in services_response.get("services", []):
            for service in category.get("items", []):
                if service.get("id") == service_id:
                    ...
                    try:
                        response = requests.get(f"http://localhost:8000/api/capabilities/check/{service_id}", timeout=2)
                        if response.status_code == 200:
