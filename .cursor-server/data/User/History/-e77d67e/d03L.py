...
async def get_dashboard_stats(current_user: dict = Depends(get_current_user)):
    ...
    def get_stats_data():
        try:
            ...
            try:
                ...
                # If direct call didn't work, try HTTP endpoint
                if security_health is None:
                    try:
                        # Use settings to get base URL instead of hardcoded localhost
                        base_url = get_settings().BACKEND_URL if hasattr(get_settings(), 'BACKEND_URL') else "http://localhost:8000"
                        response = requests.get(f"{base_url}/api/security/threat-detection/summary?hours=24", timeout=5)
                        if response.status_code == 200:
                ...
                # Also check hardening status from real endpoint (optional bonus)
                try:
                    base_url = get_settings().BACKEND_URL if hasattr(get_settings(), 'BACKEND_URL') else "http://localhost:8000"
                    hardening_response = requests.get(f"{base_url}/api/security/hardening/status", timeout=3)
    ...
    try:
        # Run with timeout (max 30 seconds to allow all endpoints to respond)
        loop = asyncio.get_event_loop()
        result = await asyncio.wait_for(
            loop.run_in_executor(None, get_stats_data),
            timeout=30.0  # Increased to 30 seconds to handle slow endpoints
        )
        return result
