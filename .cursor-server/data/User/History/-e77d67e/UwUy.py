            ...
            try:
                ...
                if security_health is None:
                    try:
                        ...
                        base_url = get_settings().BACKEND_URL if hasattr(get_settings(), 'BACKEND_URL') else "http://localhost:8000"
                        response = requests.get(f"{base_url}/api/security/threat-detection/summary?hours=24", timeout=2)  # Reduced to 2 seconds
                        if response.status_code == 200:
                ...
                try:
                    base_url = get_settings().BACKEND_URL if hasattr(get_settings(), 'BACKEND_URL') else "http://localhost:8000"
                    hardening_response = requests.get(f"{base_url}/api/security/hardening/status", timeout=1.5)  # Reduced to 1.5 seconds
