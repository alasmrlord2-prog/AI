...
if [ "$HTTP_CODE" = "200" ] || [ "$HTTP_CODE" = "401" ] || [ "$HTTP_CODE" = "403" ]; then
    echo "   ✅ Endpoint exists (HTTP $HTTP_CODE) - Router is registered"
    if [ -n "$BODY" ] && [ "$BODY" != "null" ]; then
        echo "   Response: $(echo "$BODY" | head -c 100)..."
    fi
elif [ "$HTTP_CODE" = "404" ]; then
    echo "   ❌ Router not registered (HTTP 404)"
    echo "   💡 Fix: Run ./fix-backend.sh to rebuild backend"
else
    echo "   ⚠️  HTTP $HTTP_CODE - Check logs for details"
fi
echo ""
...
if [ "$HTTP_CODE" = "200" ] || [ "$HTTP_CODE" = "401" ] || [ "$HTTP_CODE" = "403" ]; then
    ...
    echo "   ⚠️  Database error (HTTP 500)"
    echo "   💡 Fix: Run ./backend-restart.sh to restart database"
elif [ "$HTTP_CODE" = "404" ]; then
    echo "   ❌ Router not registered (HTTP 404)"
    echo "   💡 Fix: Run ./fix-backend.sh to rebuild backend"
else
