...
export default function ToolsPage() {
    ...
    try {
      ...
      if (res.ok && !data.error) {
        ...
        if (content) {
          ...
          } else if (content.includes("Errno") || content.includes("Error") || content.includes("No such file")) {
        ...
        setFileContent(`❌ Error: ${errorMsg}\n\n💡 Tip: Enable "Allow Read File" in Settings page`);
        setError(errorMsg);
      }
    } catch (err) {
  ...
  const runShell = async () => {
    ...
    try {
      ...
      const data = await res.json();
      console.log("Run Shell Response:", data);
      ...
      if (res.ok && !data.error) {
        ...
        setCmdOutput(`❌ Error: ${errorMsg}\n\n💡 Tip: Enable "Allow Shell" in Settings page`);
        setError(errorMsg);
      }
    } catch (err) {
    ...
    try {
      ...
      if (res.ok && !data.error) {
        ...
        if (typeof data.status === "string") {
          if (data.status.trim()) {
            setSvcStatus(data.status);
          } else {
            setSvcStatus(`⚠️ Service "${svc}" status is empty.\n\nThis might mean:\n- Service is not installed\n- Service name is incorrect\n- No status information available\n\nTry checking with: systemctl status ${svc}`);
          }
        } else if (typeof data.status === "object" && data.status !== null) {
          setSvcStatus(JSON.stringify(data.status, null, 2));
        } else if (data.result) {
          setSvcStatus(typeof data.result === "string" ? data.result : JSON.stringify(data.result, null, 2));
        } else {
          setSvcStatus(`⚠️ Service "${svc}" status is empty.\n\nThis might mean:\n- Service is not installed\n- Service name is incorrect\n- No status information available`);
        }
      } else {
