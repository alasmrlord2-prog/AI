...
export default function ToolsPage() {
  ...
  const runReadFile = async () => {
    ...
    try {
      ...
      const res = await fetch(`${getApiUrl()}/api/tools/read_file`, {
        method: "POST",
        headers: { 
  ...
  const runShell = async () => {
    ...
    try {
      const token = getAuthToken();
      const res = await fetch(`${getApiUrl()}/api/tools/run_shell`, {
        method: "POST",
        headers: { 
          "Content-Type": "application/json",
          ...(token && { "Authorization": `Bearer ${token}` }),
  ...
  const getResultByActionId = async (actionId: string, toolType: 'read_file' | 'run_shell' | 'service') => {
    try {
      ...
      if (toolType === 'read_file') {
        url = `${getApiUrl()}/api/tools/read_file?action_id=${actionId}`;
        body = { path };
        setLoadingRead(true);
      } else if (toolType === 'run_shell') {
        url = `${getApiUrl()}/api/tools/run_shell?action_id=${actionId}`;
        body = { cmd };
        setLoadingShell(true);
      } else if (toolType === 'service') {
        url = `${getApiUrl()}/api/tools/service?action_id=${actionId}`;
        body = { name: svc };
  ...
  const checkService = async () => {
    ...
    try {
      const token = getAuthToken();
      const res = await fetch(`${getApiUrl()}/api/tools/service`, {
        method: "POST",
        headers: { 
          "Content-Type": "application/json",
          ...(token && { "Authorization": `Bearer ${token}` }),
