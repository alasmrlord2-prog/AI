...
export default function MonitorPage() {
  ...
  const [prometheusData, setPrometheusData] = useState<PrometheusMetrics | null>(null);
      ...
      for (const query of queries) {
        try {
          ...
          if (error.name !== 'AbortError' && 
            ...
            console.debug(`Prometheus query failed for ${query}:`, error.message);
          }
        }
      }
  ...
  const fetchStatus = async () => {
    ...
    // Don't show loading if we already have data - allows fast page navigation
    if (!data) {
      setLoading(true);
    }
    
    try {
      // Longer timeout to allow backend to respond
      // If backend is slow, page still works but will try again
      const json = await apiRequest("/api/monitor", {
        method: "GET",
      }, 15000).catch((err) => {
        // Log error for debugging
        console.warn("Monitor fetch failed:", err);
        // If timeout, return null - page still works
        return null;
      });
      ...
      // CPU History
      if (json.load_avg?.["1min"] && typeof json.load_avg["1min"] === 'number') {
        setCpuHistory(prev => {
          const newData = { label: timeLabel, value: json.load_avg["1min"] };
      ...
      // Memory History
      if (json.memory && json.memory.total && json.memory.available !== undefined) {
        ...
        if (typeof memoryPercent === 'number' && !isNaN(memoryPercent)) {
          setMemoryHistory(prev => {
            const newData = { label: timeLabel, value: memoryPercent };
            const updated = [...prev, newData];
      ...
      if (json.network?.stats?.total) {
        const rxValue = json.network.stats.total.rx_mbps || 0;
        const txValue = json.network.stats.total.tx_mbps || 0;
        if (typeof rxValue === 'number') {
          setNetworkRxHistory(prev => {
            const newData = { label: timeLabel, value: rxValue };
            const updated = [...prev, newData];
            return updated.slice(-30);
          });
        }
        if (typeof txValue === 'number') {
          setNetworkTxHistory(prev => {
            const newData = { label: timeLabel, value: txValue };
            const updated = [...prev, newData];
            return updated.slice(-30);
          });
        }
      ...
      // Also fetch Prometheus metrics (don't wait for it)
      fetchPrometheusMetrics().catch(() => {
        // Ignore Prometheus errors - it's optional
      });
    } catch (err) {
      // Silently fail - page still works without data
      console.warn("Monitor data fetch failed (non-blocking):", err);
      
      // Only show error on first load if we have no data at all
      if (!data) {
        setError(null); // Don't show error - page works anyway
      }
    } finally {
      setLoading(false);
      fetchingRef.current = false;
  ...
  useEffect(() => {
    // Initial fetch - don't block page load, run in background
    // Page opens immediately, data loads when ready
    setTimeout(() => {
      fetchStatus();
    ...
    }, 15000); // 15 seconds - less frequent updates for better performance
  ...
  // Show page immediately even if loading - don't block navigation
  // Data will load in background
  // Always show page - don't block on loading or errors
  // Page works even without data
          ...
          {!data && !loading && (
            <div className="bg-yellow-900/20 border border-yellow-700 rounded-lg p-4 mb-6">
              <p className="text-yellow-400 text-sm mb-2">
                ⚠️ البيانات قيد التحميل... الصفحة تعمل بشكل طبيعي
              </p>
              <p className="text-yellow-500 text-xs mb-2">
                Backend URL: {getApiUrl()}/api/monitor
              </p>
              <button
                onClick={fetchStatus}
            ...
            <CardContent>
              <BarChart
                data={[
                  {
                    label: "Running",
                    value: Object.values(data.services).filter((s: unknown) => s === "" || s === "OK").length,
                  },
                  {
                    label: "Failed",
                    value: Object.values(data.services).filter((s: unknown) => s !== "" && s !== "OK").length,
              ...
              <div className="space-y-4">
                ...
                {data.network.stats.interfaces && Object.keys(data.network.stats.interfaces).length > 0 && (
                  <div>
                    ...
                    <div className="space-y-2">
                      {Object.entries(data.network.stats.interfaces).map(([iface, stats]: [string, unknown]) => {
                        const statsObj = stats as { rx_errors?: number; tx_errors?: number; rx_bytes?: number; tx_bytes?: number; rx_packets?: number; tx_packets?: number };
                        return (
                          <div key={iface} className="p-3 bg-slate-800/50 border border-slate-700 rounded">
                            <div className="flex items-center justify-between mb-2">
                              ...
                              {((statsObj.rx_errors || 0) > 0 || (statsObj.tx_errors || 0) > 0) && (
                                <span className="text-xs text-red-400">
                                  ⚠️ Errors: RX {statsObj.rx_errors || 0} / TX {statsObj.tx_errors || 0}
                                </span>
                              )}
                            </div>
                            <div className="grid grid-cols-2 gap-2 text-xs">
                              <div>
                                ...
                                <span className="text-cyan-400">
                                  {((statsObj.rx_bytes || 0) / 1024 / 1024).toFixed(2)} MB
                                </span>{" "}
                                <span className="text-slate-500">
                                  ({((statsObj.rx_bytes || 0) / 1024 / 1024 / 8).toFixed(2) || "0"} Mbps)
                                </span>
                              </div>
                              <div>
                                <span className="text-slate-400">TX:</span>{" "}
                                <span className="text-cyan-400">
                                  {((statsObj.tx_bytes || 0) / 1024 / 1024).toFixed(2)} MB
                                </span>{" "}
                                <span className="text-slate-500">
                                  ({((statsObj.tx_bytes || 0) / 1024 / 1024 / 8).toFixed(2) || "0"} Mbps)
                                </span>
                              </div>
                            </div>
                            <div className="text-xs text-slate-500 mt-2">
                              Packets: RX {(statsObj.rx_packets || 0).toLocaleString()} / TX {(statsObj.tx_packets || 0).toLocaleString()}
                            </div>
                          </div>
                        );
                      })}
                ...
                {data.network.alerts && data.network.alerts.length > 0 && (
                  <div>
                    ...
                    <div className="space-y-2">
                      {Array.isArray(data.network.alerts) ? data.network.alerts.map((alert: Record<string, unknown>, idx: number) => {
                        const alertObj = alert as { severity?: string; message?: string; type?: string };
                        return (
                          <div
                            key={idx}
                            className={`p-2 border rounded ${
                              alertObj.severity === "high"
                                ? "bg-red-900/30 border-red-700 text-red-300"
                                : "bg-yellow-900/30 border-yellow-700 text-yellow-300"
                            }`}
                          >
                            <div className="text-xs font-semibold">{alertObj.type}</div>
                            <div className="text-xs">{alertObj.message}</div>
                          </div>
                        );
                      }) : null}
                    </div>
