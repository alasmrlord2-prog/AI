...
export default function AAAPage() {
  ...
  const fetchStats = async () => {
    try {
      ...
      type Token = { expires_at?: string };
      const activeTokens = (tokens || []).filter((t: Token) => {
      ...
      type Session = { expires_at?: string };
      ...
      // Fetch users count
      const usersCount = await identityApi.getUsersCount().catch(() => ({ total_users: 0 }));
      ...
      setStats({
        ...
        total_users: (usersCount as { total_users?: number }).total_users || 0,
      });
    } catch (error) {
