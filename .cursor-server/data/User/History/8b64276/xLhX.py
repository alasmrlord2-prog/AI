...
def _call_ollama(
    ...
    - timeout: وقت الانتظار قبل الـ timeout (مخفض لسرعة الرد)
    """
...
def llm_text(user_prompt: str) -> str:
    ...
    نسمح بعدد توكينات معقول (128) لسرعة الرد.
    """
    data = _call_ollama(
        user_prompt,
        temperature=0.4,
        num_predict=128,  # Reduced from 256 to 128 for faster response
...
"""
    ...
    data = _call_ollama(
        full_prompt,
        temperature=0.1,
        num_predict=64,  # Reduced from 128 to 64 for faster JSON responses
        timeout=30,  # Reduced from 300 to 30 seconds for JSON
    )
