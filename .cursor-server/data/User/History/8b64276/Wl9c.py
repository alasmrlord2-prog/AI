...
def _call_ollama(
    ...
    num_predict: int = 256,
    timeout: int = 180,  # Increased to 180 seconds (3 minutes) to allow complex questions
) -> dict:
    """
    ...
    try:
        ...
        raise ConnectionError(f"Ollama request timed out after {timeout} seconds. The model may be too slow.")
...
def llm_text(user_prompt: str) -> str:
    ...
    نسمح بعدد توكينات معقول لضمان إجابة كاملة.
    """
    data = _call_ollama(
        user_prompt,
        temperature=0.4,
        num_predict=512,  # Increased to allow longer, more complete responses
        timeout=180,  # Increased to 180 seconds to ensure all questions are answered
    )
...
"""
    ...
    data = _call_ollama(
        full_prompt,
        temperature=0.1,
        num_predict=256,  # Increased to allow complete JSON responses
        timeout=120,  # Increased to 120 seconds for JSON to ensure complete responses
    )
