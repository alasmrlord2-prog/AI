...
export default function MonitoringPage() {
  ...
  const fetchStatus = async () => {
    ...
    try {
      // OPTIMIZED: Increased timeout to 15 seconds for better reliability
      ...
      if (json) {
        ...
        // Calculate CPU percent from load_avg (using 1min load as CPU indicator)
        if (json.load_avg && typeof json.load_avg === 'object' && json.load_avg['1min'] !== undefined) {
          const cpuValue = typeof json.load_avg['1min'] === 'number' ? json.load_avg['1min'] * 100 : 0;
          setCpuHistory((prev) => {
            const newData = { label: now, value: cpuValue };
            const updated = [...prev, newData];
            return updated.slice(-30); // Keep last 30 points
          });
        } else if (json.load_avg && json.load_avg['1min'] === 0) {
          // Even if load is 0, add it to history so chart shows
          setCpuHistory((prev) => {
            const newData = { label: now, value: 0 };
            const updated = [...prev, newData];
            return updated.slice(-30);
          });
        }
        
        // Calculate memory percent from memory object
        if (json.memory && typeof json.memory === 'object') {
          let memTotal = 0;
          let memAvailable = 0;
          
          // Handle string values from /proc/meminfo (e.g., "1234567 kB")
          if (typeof json.memory.total === 'string') {
            memTotal = parseInt(json.memory.total.replace(/[^0-9]/g, '')) || 0;
          } else if (typeof json.memory.total === 'number') {
            memTotal = json.memory.total;
          }
          
          if (typeof json.memory.available === 'string') {
            memAvailable = parseInt(json.memory.available.replace(/[^0-9]/g, '')) || 0;
          } else if (typeof json.memory.available === 'number') {
            memAvailable = json.memory.available;
          }
          
          if (memTotal > 0) {
            const memoryPercent = ((memTotal - memAvailable) / memTotal) * 100;
            setMemoryHistory((prev) => {
              const newData = { label: now, value: memoryPercent };
              const updated = [...prev, newData];
              return updated.slice(-30); // Keep last 30 points
            });
          }
        ...
        // Calculate disk percent from disk object
        if (json.disk && typeof json.disk === 'object' && json.disk.total_gb && json.disk.used_gb) {
          const diskPercent = (json.disk.used_gb / json.disk.total_gb) * 100;
          setDiskHistory((prev) => {
            const newData = { label: now, value: diskPercent };
            const updated = [...prev, newData];
            return updated.slice(-30); // Keep last 30 points
          });
        }
        
        // Network data from network object
        if (json.network && typeof json.network === 'object') {
          // Try different possible structures
          const rxValue = json.network.rx_mbps || json.network.stats?.total?.rx_mbps || json.network.rx || 0;
          const txValue = json.network.tx_mbps || json.network.stats?.total?.tx_mbps || json.network.tx || 0;
          
          if (typeof rxValue === 'number' && rxValue >= 0) {
            setNetworkRxHistory((prev) => {
              const newData = { label: now, value: rxValue };
              const updated = [...prev, newData];
              return updated.slice(-30); // Keep last 30 points
            });
          }
          
          if (typeof txValue === 'number' && txValue >= 0) {
            setNetworkTxHistory((prev) => {
              const newData = { label: now, value: txValue };
              const updated = [...prev, newData];
              return updated.slice(-30); // Keep last 30 points
            });
          }
            ...
            <>
                ...
                <Card className="bg-slate-900 border-slate-800">
                  <CardContent className="p-3 md:p-4">
                    ...
                    <div className="text-2xl md:text-3xl font-bold mt-1 md:mt-2 text-blue-400">
                      {data?.load_avg?.['1min'] ? (data.load_avg['1min'] * 100).toFixed(1) : "0"}%
                    </div>
                  </CardContent>
                </Card>
                <Card className="bg-slate-900 border-slate-800">
                    ...
                    <div className="text-2xl md:text-3xl font-bold mt-1 md:mt-2 text-purple-400">
                      {(() => {
                        if (data?.memory && typeof data.memory === 'object') {
                          let memTotal = 0;
                          let memAvailable = 0;
                          if (typeof data.memory.total === 'string') {
                            memTotal = parseInt(data.memory.total.replace(/[^0-9]/g, '')) || 0;
                          } else if (typeof data.memory.total === 'number') {
                            memTotal = data.memory.total;
                          }
                          if (typeof data.memory.available === 'string') {
                            memAvailable = parseInt(data.memory.available.replace(/[^0-9]/g, '')) || 0;
                          } else if (typeof data.memory.available === 'number') {
                            memAvailable = data.memory.available;
                          }
                          if (memTotal > 0) {
                            return ((memTotal - memAvailable) / memTotal * 100).toFixed(1);
                          }
                        }
                        return "0";
                      })()}%
                ...
                <Card className="bg-slate-900 border-slate-800">
                  <CardContent className="p-3 md:p-4">
                    ...
                    <div className="text-2xl md:text-3xl font-bold mt-1 md:mt-2 text-green-400">
                      {data?.disk?.total_gb && data?.disk?.used_gb 
                        ? ((data.disk.used_gb / data.disk.total_gb) * 100).toFixed(1) 
                        : "0"}%
                    </div>
                  </CardContent>
                </Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                <Card className="bg-slate-900 border-slate-800">
                  ...
                  <CardContent>
                    {cpuHistory.length > 0 ? (
                      <LineChart data={cpuHistory} color="var(--sw-blue)" height={200} />
                    ) : (
                ...
                <Card className="bg-slate-900 border-slate-800">
                  ...
                  <CardContent>
                    {memoryHistory.length > 0 ? (
                      <LineChart data={memoryHistory} color="var(--sw-teal-light)" height={200} />
                    ) : (
                      <div className="h-[200px] flex items-center justify-center text-slate-500">
                        No data available
                      </div>
                ...
                <Card className="bg-slate-900 border-slate-800">
                  ...
                  <CardContent>
                    {diskHistory.length > 0 ? (
                      <LineChart data={diskHistory} color="var(--sw-teal)" height={200} />
                    ) : (
                      <div className="h-[200px] flex items-center justify-center text-slate-500">
                        No data available
                      </div>
                    ...
                    {networkRxHistory.length > 0 || networkTxHistory.length > 0 ? (
                      <div className="space-y-4">
                        <div>
                          ...
                          <LineChart data={networkRxHistory} color="var(--sw-blue-light)" height={90} />
                        </div>
                        <div>
                          <div className="text-sm text-slate-400 mb-2">TX</div>
                          <LineChart data={networkTxHistory} color="var(--sw-primary-dark)" height={90} />
                        </div>
                      </div>
                    ) : (
                      <div className="h-[200px] flex items-center justify-center text-slate-500">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                    <div className="p-3 md:p-4 bg-slate-800 rounded">
                      ...
                      <div className="text-xl md:text-2xl font-bold mt-2 text-cyan-400">
                        {(() => {
                          const rx = data?.network?.rx_mbps || data?.network?.stats?.total?.rx_mbps || data?.network?.rx || 0;
                          return typeof rx === 'number' ? rx.toFixed(2) : "0.00";
                        })()}
                      </div>
                    </div>
                    <div className="p-3 md:p-4 bg-slate-800 rounded">
                      ...
                      <div className="text-xl md:text-2xl font-bold mt-2 text-orange-400">
                        {(() => {
                          const tx = data?.network?.tx_mbps || data?.network?.stats?.total?.tx_mbps || data?.network?.tx || 0;
                          return typeof tx === 'number' ? tx.toFixed(2) : "0.00";
                        })()}
                      </div>
                    </div>
                    <div className="p-3 md:p-4 bg-slate-800 rounded">
                  ...
                  {networkRxHistory.length > 0 || networkTxHistory.length > 0 ? (
                    <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                      <div>
                        ...
                        <LineChart data={networkRxHistory} color="var(--sw-blue-light)" height={150} />
                      </div>
                      <div>
                        <div className="text-sm text-slate-400 mb-2">TX Trend</div>
                        <LineChart data={networkTxHistory} color="var(--sw-primary-dark)" height={150} />
                      </div>
                    </div>
                  ) : null}
                </CardContent>
