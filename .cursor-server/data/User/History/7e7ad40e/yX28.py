...
def _decide_action(user_message: str, history) -> dict:
    ...
    if any(keyword in user_lower for keyword in ["service", "check", "status", "systemctl"]):
        ...
        service_keywords = ["nginx", "apache", "mysql", "postgres", "redis", "docker", "ssh", "ollama"]
        detected_service = None
        
        # Look for service name in the message
        for i, word in enumerate(words):
            # Check if word is a service keyword
            if word in service_keywords:
                detected_service = word
                break
            # Check for "check <service>" or "status <service>" pattern
            if word in ["check", "status"] and i + 1 < len(words):
                next_word = words[i + 1]
                if next_word not in ["service", "of", "the"]:
                    detected_service = next_word
                    break
    ...
    if decision["mode"] == "tool" and decision.get("tool_name") in TOOLS:
        ...
        try:
            ...
            if tool_name == "run_shell":
                ...
                if isinstance(tool_args, dict):
                    ...
                    output = tool_fn(str(service_name))
                else:
                    if not tool_args:
        ...
        if tool_name == "run_shell":
            ...
            if tool_name == "check_service":
                # Use output from check_service directly - it already contains proper status
                # Only add formatting if output doesn't already indicate status clearly
                output_lower = output_str.lower()
                
                # Check if output already contains clear status indicators
                has_status = any(indicator in output_lower for indicator in [
                    "running", "active", "inactive", "stopped", "not found", 
                    "not running", "found in docker", "appears to be running"
                ])
                
                # If output already has clear status, use it as-is (no hardcoded messages)
                if has_status:
                    final = output_str
                else:
                    # Only format if output is unclear
