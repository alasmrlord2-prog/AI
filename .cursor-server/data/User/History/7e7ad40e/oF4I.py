    ...
    if decision["mode"] == "tool" and decision.get("tool_name") in TOOLS:
        ...
        if tool_name == "run_shell":
            ...
            try:
                ...
                logger.info(f"[Agent] Calling llm_text for tool follow-up")
                final = llm_text(follow_prompt).strip()
                if not final:
                    logger.warning(f"[Agent] Tool follow-up returned empty, using tool output")
                    final = output_str
            except Exception as e:
                logger.error(f"[Agent] Error in follow-up LLM call: {e}")
