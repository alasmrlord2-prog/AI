    ...
    if decision["mode"] == "tool" and decision.get("tool_name") in TOOLS:
        ...
        try:
            ...
            if tool_name == "run_shell":
                ...
                # Store service name in tool_args for later use
        ...
        if tool_name == "run_shell":
            ...
            if tool_name == "check_service":
                # Get service name for detailed explanation
                service_name = str(tool_args) if not isinstance(tool_args, dict) else (
                    tool_args.get("service") or tool_args.get("svc") or list(tool_args.values())[0] if tool_args else "service"
                )
