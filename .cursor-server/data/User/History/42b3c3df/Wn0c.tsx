...
export default function Sidebar() {
  ...
  const featureKeyForPath = (href: string) => {
    ...
    const map: Record<string, string> = {
      ...
      "logs": "monitor",
      "incidents": "monitor",
      "incident-center": "monitor",
      "siem": "siem",
      "soc": "siem",
      "security": "siem",
      "threat-detection": "siem",
      "behavior-alerts": "siem",
      "abac": "security",
      "tools": "tools",
      "workflows": "workflows",
  ...
  const isFeatureEnabled = useMemo(() => {
    return (href: string) => {
      if (!tenantFeatures || Object.keys(tenantFeatures).length === 0) {
        return true;
      }
      const featureKey = featureKeyForPath(href);
      const feature = tenantFeatures[featureKey];
    ...
    const filterItems = (items: MenuItem[]): MenuItem[] =>
      items
        .map((item) => {
          ...
          const enabled = isFeatureEnabled(item.href);
    ...
    >
      ...
      <div className="flex-shrink-0 px-6 py-5 border-b border-sw-border">
        <Link href="/" className="flex items-center gap-3 hover:opacity-80 transition-opacity cursor-pointer">
          <Image 
            src="/shiftwave-logo-light.svg" 
            alt="SHIFTWAVE Logo" 
            width={36}
            height={36}
            className="h-9 w-auto flex-shrink-0 sw-logo sw-logo-light dark:hidden"
          />
          <Image 
            src="/shiftwave-logo-dark.svg" 
            alt="SHIFTWAVE Logo" 
            width={36}
            height={36}
            className="h-9 w-auto flex-shrink-0 sw-logo sw-logo-dark hidden dark:block"
          />
        </Link>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto overflow-x-hidden sidebar-scroll px-4 py-4">
        <div className="space-y-6">
          ...
          <div className="space-y-1.5 pt-4 border-t border-sw-border">
            <h3 className="text-[10px] font-bold text-sw-text-muted uppercase tracking-wider px-3 py-1.5">
              EXTERNAL
            </h3>
            <div className="space-y-0.5">
              <a
                ...
                className="block px-3 py-2 rounded-sw-btn transition-all duration-200 text-sw-text-muted hover:text-sw-text hover:bg-sw-bg-hover group"
              >
                <span className="flex items-center gap-2.5">
                  <Building2 className="w-4 h-4" />
                  <span className="text-sm font-medium">CRM Portal</span>
                </span>
              </a>
              <a
                href="http://aaa.bankid-sy.com"
                target="_blank"
                rel="noopener noreferrer"
                className="block px-3 py-2 rounded-sw-btn transition-all duration-200 text-sw-text-muted hover:text-sw-text hover:bg-sw-bg-hover group"
              >
                <span className="flex items-center gap-2.5">
                  ...
                  <span className="text-sm font-medium">AAA Portal</span>
                </span>
              </a>
            </div>
          </div>
