...
export default function ServicesDrawer({
  ...
  return (
    <>
      ...
      <div
        className="fixed top-0 left-0 h-full w-80 bg-[#1a1f2e] dark:bg-[#0a0f16] shadow-xl border-r border-gray-700 dark:border-gray-800 z-[60]"
        style={{
          animation: "slide-in-left 0.3s ease-out",
        }}
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-700 dark:border-gray-800">
          <h2 className="font-semibold text-lg text-white dark:text-gray-100">All Services</h2>
          <button
            onClick={onClose}
  ...
  return (
    <div className="flex items-center justify-between group">
      <Link
        href={item.href}
        onClick={onNavigate}
        className="flex items-center gap-2 px-2 py-1.5 rounded-md text-sm hover:bg-gray-700 dark:hover:bg-gray-800 flex-1 transition-colors text-white dark:text-gray-100"
      >
        <span className="text-base w-6 text-center flex-shrink-0">{item.icon}</span>
        <span className="text-white dark:text-gray-100">{item.name}</span>
      </Link>
      <button
        onClick={(e) => {
          e.preventDefault();
        ...
        className="ml-1 text-xs px-1.5 py-0.5 text-yellow-400 hover:text-yellow-300 dark:text-yellow-500 dark:hover:text-yellow-400 transition-colors flex-shrink-0"
