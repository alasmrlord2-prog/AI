...
async def api_threat_intelligence(
    ...
    try:
        result = threat_intelligence_scan(ip)
...
async def api_scan_port_scan(
    ...
    try:
        check_action_permission("security.scan", current_user, resource=target)
    except HTTPException as e:
        if e.status_code == 202:
            return JSONResponse(status_code=202, content={"error": e.detail})
        raise
...
async def api_web_scan(
    ...
    try:
        check_action_permission("security.scan", current_user, resource=url)
    except HTTPException as e:
        if e.status_code == 202:
            return JSONResponse(status_code=202, content={"error": e.detail})
        raise
...
async def api_packet_analysis(
    ...
    try:
        check_action_permission("security.scan", current_user, resource=file_path or "")
    except HTTPException as e:
        if e.status_code == 202:
            return JSONResponse(status_code=202, content={"error": e.detail})
        raise
