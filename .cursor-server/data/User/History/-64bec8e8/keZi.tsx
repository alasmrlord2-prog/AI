...
export default function CICDPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [reposRes, pipelinesRes] = await Promise.all([
        fetch(`${getApiUrl()}/api/cicd/repos`),
        fetch(`${getApiUrl()}/api/cicd/pipelines`)
      ]);
  ...
  const handleClone = async () => {
    ...
    try {
      const res = await fetch(`${getApiUrl()}/api/cicd/clone`, {
        method: "POST",
  ...
  const handleRunPipeline = async (repoName: string) => {
    try {
      const res = await fetch(`${getApiUrl()}/api/cicd/run`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handlePull = async (repoName: string) => {
    try {
      const res = await fetch(`${getApiUrl()}/api/cicd/pull`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const fetchPipelineLogs = async (pipelineId: string) => {
    try {
      const res = await fetch(`${getApiUrl()}/api/cicd/logs/${pipelineId}?tail=100`);
      if (res.ok) {
        const data = await res.json();
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      if (deployType === "docker-compose") {
        endpoint = `${getApiUrl()}/api/cicd/deploy/docker-compose`;
        body = { compose_file: deployFile };
      } else if (deployType === "kubernetes") {
        endpoint = `${getApiUrl()}/api/cicd/deploy/kubernetes`;
        body = { manifest_file: deployFile };
      } else if (deployType === "rsync") {
        endpoint = `${getApiUrl()}/api/cicd/deploy/rsync`;
  ...
  const handleRollback = async (pipelineId: string) => {
    ...
    try {
      const res = await fetch(`${getApiUrl()}/api/cicd/rollback/${pipelineId}`, {
        method: "POST"
      });
