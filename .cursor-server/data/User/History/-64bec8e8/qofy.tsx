    ...
    try {
      if (tab === "cicd") {
        const [pipelinesData, jobsData] = await Promise.all([
          apiRequest("/api/cicd/pipelines", {}, 3000).catch(() => ({ pipelines: [] })),
          apiRequest("/api/cicd/jobs", {}, 3000).catch(() => ({ jobs: [] }))
        ]);
        setPipelines(pipelinesData.pipelines || []);
        setRunningJobs(jobsData.jobs?.filter((j: any) => j.status === "running") || []);
      } else if (tab === "deployments") {
        const [deployData, shadowData] = await Promise.all([
          apiRequest("/api/cicd/deployments", {}, 3000).catch(() => ({ deployments: [] })),
          apiRequest("/api/shadow-deploy/list", {}, 3000).catch(() => ({ deployments: [] }))
        ]);
        setDeployments(deployData.deployments || []);
        setShadowDeployments(shadowData.deployments || []);
      } else if (tab === "backup") {
                  ...
                  <div className="space-y-2">
                    {pipelines.length > 0 ? (
                      pipelines.map((pipeline, idx) => (
                        <div key={`pipeline-${pipeline.id || pipeline.name || idx}-${idx}`} className="p-4 bg-slate-800 rounded border-l-4 border-blue-500">
                  ...
                  <div className="space-y-2">
                    {runningJobs.length > 0 ? (
                      runningJobs.map((job, idx) => (
                        <div key={`job-${job.id || job.name || idx}-${idx}`} className="p-4 bg-slate-800 rounded border-l-4 border-yellow-500">
                          <div className="flex justify-between items-start">
                            <div>
                ...
                <CardContent>
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    {pipelines.slice(0, 20).map((pipeline, idx) => (
                      <div key={`history-${pipeline.id || pipeline.name || idx}-${idx}`} className="p-3 bg-slate-800 rounded text-sm">
                  ...
                  <div className="space-y-2">
                    {deployments.length > 0 ? (
                      deployments.map((deployment, idx) => (
                        <div key={`deployment-${deployment.id || deployment.name || idx}-${idx}`} className="p-4 bg-slate-800 rounded border-l-4 border-green-500">
                          <div className="flex justify-between items-start">
                            <div>
                  ...
                  <div className="space-y-2">
                    {shadowDeployments.length > 0 ? (
                      shadowDeployments.map((deployment, idx) => (
                        <div key={`shadow-${deployment.id || deployment.name || idx}-${idx}`} className="p-4 bg-slate-800 rounded border-l-4 border-purple-500">
                          <div className="flex justify-between items-start">
                            <div>
                  ...
                  <div className="space-y-2">
                    {backups.length > 0 ? (
                      backups.map((backup, idx) => (
                        <div key={`backup-${backup.id || backup.name || idx}-${idx}`} className="p-4 bg-slate-800 rounded">
                          <div className="flex justify-between items-start">
                            <div>
