...
export default function CICDPage() {
  ...
  const fetchPipelineLogs = async (pipelineId: string) => {
    try {
      const res = await fetch(`${API_URL}/api/cicd/logs/${pipelineId}?tail=100`);
      if (res.ok) {
        const data = await res.json();
  ...
  const handleRollback = async (pipelineId: string) => {
    if (!confirm("هل أنت متأكد من Rollback لهذا Pipeline?")) return;

    try {
      const res = await fetch(`${API_URL}/api/cicd/rollback/${pipelineId}`, {
        method: "POST"
      });
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      if (deployType === "docker-compose") {
        endpoint = `${API_URL}/api/cicd/deploy/docker-compose`;
        body = { compose_file: deployFile };
      } else if (deployType === "kubernetes") {
        endpoint = `${API_URL}/api/cicd/deploy/kubernetes`;
        body = { manifest_file: deployFile };
      } else if (deployType === "rsync") {
        endpoint = `${API_URL}/api/cicd/deploy/rsync`;
        body = { source: deployFile, destination: "/var/www" };
      }
            ...
            {showDeployForm && (
              <Card className="mb-6 border-purple-200 bg-purple-50">
                ...
                <CardContent className="space-y-4">
                  <div>
                    <Label htmlFor="deployType">Deploy Type</Label>
                    <select
                      id="deployType"
                      value={deployType}
            ...
            {selectedPipeline && (
              <Card className="mt-6 border-slate-200 shadow-lg">
                <CardHeader className="bg-gradient-to-r from-gray-500 to-gray-600 text-white rounded-t-lg">
                  <div className="flex justify-between items-center">
                    <CardTitle className="text-white">Pipeline Logs: {selectedPipeline}</CardTitle>
                    <Button
                      size="sm"
                      variant="outline"
                ...
                <CardContent className="p-4">
                  <div className="bg-slate-900 text-green-400 p-4 rounded font-mono text-sm max-h-96 overflow-y-auto">
                    {pipelineLogs.length === 0 ? (
                      <p className="text-slate-500">لا توجد logs</p>
                    ) : (
