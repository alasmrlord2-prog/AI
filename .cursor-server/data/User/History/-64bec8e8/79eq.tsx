...
export default function CICDPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (repos.length === 0 && pipelines.length === 0) {
        setLoading(true);
      }
      const [reposData, pipelinesData] = await Promise.all([
        apiRequest("/api/cicd/repos", {}, 3000).catch(() => ({ repos: [] })), // Faster timeout
  ...
  const handleRunPipeline = async (repoName: string) => {
    try {
      ...
      }, 30000); // 30s timeout for pipeline run
  ...
  const fetchPipelineLogs = async (pipelineId: string) => {
    try {
      const data = await apiRequest(`/api/cicd/logs/${pipelineId}?tail=100`, {}, 5000);
  ...
  const handleRollback = async (pipelineId: string) => {
    ...
    try {
      await apiRequest(`/api/cicd/rollback/${pipelineId}`, {
        method: "POST"
      }, 30000); // 30s timeout for rollback
