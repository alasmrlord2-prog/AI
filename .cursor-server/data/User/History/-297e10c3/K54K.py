...
class CRMService:
    ...
    def get_tenant_incidents(
        ...
        # Import incidents model if available
        try:
            from app.monitoring.models import Incident
            incidents = db.query(Incident).filter(
                Incident.tenant_id == tenant_id
            ).order_by(Incident.created_at.desc()).offset(offset).limit(limit).all()
            ...
            for incident in incidents:
                incidents_data.append({
                    ...
                    "resolved_at": getattr(incident, "resolved_at", None).isoformat() if hasattr(incident, "resolved_at") and getattr(incident, "resolved_at") else None
                })

            return incidents_data, total
        except ImportError:
            # If incidents model doesn't exist, return empty
            return [], 0
