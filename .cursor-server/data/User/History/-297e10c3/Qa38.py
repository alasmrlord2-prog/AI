...
class CRMService:
    ...
    def get_tenant_incidents(
        ...
        # Try to import incidents model if available
        try:
            # Check if incidents table exists by trying to query it
            # This is a flexible approach that works even if the model structure varies
            from sqlalchemy import inspect
            from app.core.database import engine
            
            inspector = inspect(engine)
            if "incidents" in inspector.get_table_names():
                # Use raw SQL or try to get the model
                result = db.execute(
                ...
                for incident in incidents:
                    incidents_data.append({
                        "id": str(incident[0]) if incident[0] else None,
                        "title": incident[1] if len(incident) > 1 else "Unknown",
                        "severity": incident[2] if len(incident) > 2 else "medium",
                        "status": incident[3] if len(incident) > 3 else "open",
                        "created_at": incident[4].isoformat() if len(incident) > 4 and incident[4] else None,
                        "resolved_at": incident[5].isoformat() if len(incident) > 5 and incident[5] else None
                    })
            ...
            # If incidents table doesn't exist or any error, return empty
            return [], 0
