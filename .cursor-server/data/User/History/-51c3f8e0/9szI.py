...
async def chat(req: ChatRequest):
    ...
    try:
        ...
        try:
            ...
            # Run think_and_act in executor with timeout (max 300 seconds for chat)
            # Increased timeout to allow agent to answer all questions, even complex ones
            loop = asyncio.get_event_loop()
            reply = await asyncio.wait_for(
                loop.run_in_executor(None, think_and_act, req.message.strip()),
                timeout=300.0  # 300 second timeout (5 minutes) to ensure all questions are answered
            )
            ...
            logger.error(f"[Chat] Timeout for session {session_id} after 300 seconds")
            log_chat("error", session_id, error_msg)
            # Try one more time with a simpler approach before giving up
            try:
                logger.info(f"[Chat] Attempting fallback response for session {session_id}")
                from app.agent.core_llm import llm_text
                reply = llm_text(f"أجب على السؤال التالي بشكل مختصر: {req.message.strip()}")
                if not reply or not reply.strip():
                    reply = "⚠️ استغرقت العملية وقتاً طويلاً جداً. يرجى المحاولة مرة أخرى أو تبسيط السؤال."
