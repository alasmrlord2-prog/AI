...
if [ "$EUID" -eq 0 ] || [ "$(id -u)" -eq 0 ]; then
    ...
    # Try docker first, fallback to sudo only if needed
    if docker ps >/dev/null 2>&1; then
        DOCKER_CMD="docker"
        echo "✅ Docker accessible - using docker directly"
    else
        DOCKER_CMD="docker"  # Still try docker first
        echo "⚠️  Will try docker first, may need to add user to docker group"
    fi
fi
...
# Kill processes on ports (multiple attempts)
...
for port in 3000 3001 3002; do
    ...
    if [ -n "$PIDS" ]; then
        echo "$PIDS" | xargs kill -9 2>/dev/null && echo "      ✅ Killed remaining process on port $port" || true
    fi
done
