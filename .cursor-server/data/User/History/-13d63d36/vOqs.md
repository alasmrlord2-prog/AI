...
- **الأدوات المستخدمة:**
  ...
  - Python `socket` library - للاتصال بالمنافذ
  - `nc` (netcat) - كبديل
  - TCP connection testing - اختبار الاتصالات
...
- **الأدوات المستخدمة:**
  ...
  - `find` - للبحث عن ملفات قابلة للكتابة
  - SSH configuration analysis - تحليل تكوين SSH
  - Exploit testing - اختبار الاستغلال
  - Security configuration review - مراجعة التكوينات الأمنية
...
### أدوات التحليل والمراقبة (Analysis & Monitoring Tools)

#### 1. Log Aggregator
- **الوظيفة:** تجميع السجلات من مصادر متعددة
- **الأدوات المستخدمة:**
  - File reading - قراءة الملفات
  - Log parsing - تحليل السجلات
...
#### 3. Alert System
- **الوظيفة:** نظام التنبيهات
- **الأدوات المستخدمة:**
  - Alert generation - توليد التنبيهات
  - Notification channels - قنوات الإشعارات
  - Threshold monitoring - مراقبة العتبات
...
- **الأدوات المستخدمة:**
  ...
  - Python `subprocess` - لتشغيل الأوامر
  - Process monitoring - لمراقبة العمليات
...
- **الأدوات المستخدمة:**
  ...
  - Path management - إدارة المسارات
  - `shutil` - لإدارة الملفات والمجلدات
  - `pathlib` - للتعامل مع المسارات

#### 3. Build Tools
- **الوظيفة:** بناء التطبيقات
- **الأدوات المستخدمة:**
  - `docker build` - لبناء صور Docker
  - `npm build` / `yarn build` - لبناء تطبيقات Node.js
  - `maven` / `gradle` - لبناء تطبيقات Java
  - `make` - لتشغيل ملفات Makefile
  - Compiler tools - أدوات الترجمة

#### 4. Testing Tools
- **الوظيفة:** تشغيل الاختبارات
- **الأدوات المستخدمة:**
  - `pytest` - لاختبارات Python
  - `jest` / `mocha` - لاختبارات JavaScript
  - `junit` - لاختبارات Java
  - Test runners - مشغلات الاختبارات
...
- **الأدوات المستخدمة:**
  - Python `subprocess` module - لإدارة العمليات
  - Process monitoring - مراقبة العمليات
  - Log collection - جمع السجلات
  - Timeout management - إدارة المهلات الزمنية
  - Error handling - معالجة الأخطاء

#### 3. Environment Adapter (`env_adapter.py`)
- **الوظيفة:** التكيف مع البيئة المختلفة
- **الأدوات المستخدمة:**
  - Tool detection - كشف الأدوات المتاحة
  - Command execution - تنفيذ الأوامر
  - Environment variables - متغيرات البيئة
  - Capability detection - كشف القدرات

#### 4. Capability Detector (`capability_detector.py`)
- **الوظيفة:** كشف القدرات المتاحة في النظام
- **الأدوات المستخدمة:**
  - Tool availability checking - فحص توفر الأدوات
...
- **الأدوات المستخدمة:**
  - Think-and-Act framework - إطار التفكير والعمل
  - Tool calling - استدعاء الأدوات
  - Decision making - اتخاذ القرارات
  - Memory management - إدارة الذاكرة
  - LLM integration - التكامل مع نماذج اللغة
...
الذاكرة الطويلة: {long_mem}
الذاكرة القصيرة: {memory[-5:]}
مهمة المستخدم: {user_input}
...
#### 2. Tool Integration
- **الأدوات المتكاملة:**
  - Security scanners - ماسحات الأمان
  ...
  - Log reading - قراءة السجلات
  - Documentation search - البحث في الوثائق
...
- **الأدوات المستخدمة:**
  ...
  - Best practices checking - فحص أفضل الممارسات
  - LLM-powered analysis - تحليل مدعوم بالذكاء الاصطناعي
  - Pattern detection - كشف الأنماط

#### 4. Memory Management
- **الوظيفة:** إدارة ذاكرة الوكيل
- **الأدوات المستخدمة:**
  - Short-term memory - الذاكرة قصيرة المدى (`memory/memory.json`)
  - Long-term memory - الذاكرة طويلة المدى (`memory/long_memory.json`)
  - JSON storage - التخزين بصيغة JSON
  - Context management - إدارة السياق

#### 5. Tool Registry
- **الأدوات المسجلة:**
  - `read_file` - قراءة الملفات
  - `check_service` - فحص الخدمات
  - `run_shell` - تشغيل أوامر Shell
...
5. **التخزين المؤقت:** معظم الأدوات تستخدم التخزين المؤقت (caching) لتحسين الأداء
6. **معالجة الأخطاء:** جميع الأدوات محمية بـ `@handle_scan_errors` لمعالجة الأخطاء بشكل آمن

---

## 🔍 أمثلة على الاستخدام

### مثال 1: فحص المستودع للبحث عن الأسرار
```python
from app.tools.security_scanners.repo_scanner import scan_repo

# فحص المستودع
results = scan_repo(path="/app", max_files=1000)

# عرض النتائج
print(f"تم العثور على {results['summary']['secrets_count']} سر")
print(f"نقاط المخاطر: {results['risk_score']}")
```

### مثال 2: نشر باستخدام Docker Compose
```python
...
### مثال 3: استخدام الوكيل الذكي
```python
from app.agent.agent_core import think_and_act

# طلب من الوكيل
response = think_and_act("فحص حالة الخدمات")

# الوكيل سيفكر ويستخدم الأدوات المناسبة
print(response)
```

### مثال 4: فحص الثغرات
```python
from app.tools.security_scanners.vulnerability_scanner import scan_vulnerabilities

# فحص الثغرات
results = scan_vulnerabilities()

# عرض الثغرات الحرجة
critical = [v for v in results["vulnerabilities"] if v["severity"] == "critical"]
print(f"تم العثور على {len(critical)} ثغرة حرجة")
```
...
### Security Service
1. **فحص منتظم:** قم بفحص المستودعات بانتظام للبحث عن الأسرار
2. **تحديث القواعد:** حدث أنماط البحث عن الأسرار بانتظام
3. **مراقبة مستمرة:** استخدم أدوات المراقبة لاكتشاف التهديدات في الوقت الفعلي
4. **تقييم المخاطر:** استخدم نقاط المخاطر لتحديد الأولويات

### DevOps Service
1. **اختبار قبل النشر:** اختبر التطبيقات قبل النشر
2. **نسخ احتياطية:** احتفظ بنسخ احتياطية قبل النشر
3. **مراقبة النشر:** راقب حالة النشر بعد اكتماله
4. **إدارة الإصدارات:** استخدم نظام إدارة إصدارات مناسب

### AI & Automation Service
1. **ذاكرة فعالة:** استخدم الذاكرة قصيرة وطويلة المدى بفعالية
2. **أدوات مناسبة:** اختر الأداة المناسبة للمهمة
3. **معالجة الأخطاء:** تعامل مع الأخطاء بشكل صحيح
4. **تحسين الأداء:** استخدم التخزين المؤقت لتحسين الأداء

---

## 🛠 استكشاف الأخطاء

### مشكلة: أداة غير متوفرة
**الحل:**
- تحقق من تثبيت الأداة: `which <tool_name>`
- استخدم `capability_detector` للتحقق من القدرات
- استخدم البدائل المتوفرة

### مشكلة: صلاحيات غير كافية
**الحل:**
- تحقق من الصلاحيات المطلوبة
- استخدم `sudo` عند الحاجة (بحذر)
- راجع إعدادات الأمان

### مشكلة: فشل في الاتصال بـ Ollama
**الحل:**
- تحقق من أن Ollama يعمل: `curl http://localhost:11434/api/tags`
- تحقق من متغيرات البيئة: `OLLAMA_URL`
- تأكد من توفر النموذج المطلوب

### مشكلة: فشل في النشر
**الحل:**
- تحقق من ملفات التكوين (docker-compose.yml, k8s manifests)
- تحقق من توفر الموارد (ذاكرة، مساحة قرص)
- راجع السجلات للبحث عن الأخطاء
...
### Security Service
- [ ] دعم المزيد من أنماط الأسرار
- [ ] تكامل مع قواعد بيانات CVE
- [ ] دعم فحص تطبيقات الويب المتقدم
- [ ] تكامل مع أدوات الأمان الخارجية

### DevOps Service
- [ ] دعم المزيد من منصات النشر
- [ ] تحسين إدارة الخطوط الأنابيب
- [ ] دعم النشر المتعدد البيئات
- [ ] تحسين المراقبة والتنبيهات

### AI & Automation Service
- [ ] دعم المزيد من نماذج LLM
- [ ] تحسين إدارة الذاكرة
- [ ] دعم المزيد من الأدوات
- [ ] تحسين قدرات التفكير
...
تم توثيق جميع الأدوات المستخدمة في الخدمات الثلاثة:
- ✅ **Security Service:** 12 أداة فحص أساسية + أدوات متقدمة
- ✅ **DevOps Service:** أدوات Git، CI/CD، والنشر (Docker, Kubernetes, RSync)
- ✅ **AI & Automation Service:** تكامل Ollama، Agent Core، وأدوات الأتمتة

الملف الآن مكتمل ويحتوي على:
- 📝 توثيق شامل لجميع الأدوات
- 💻 أمثلة كود لكل أداة رئيسية
- 🎯 أفضل الممارسات
- 🔍 أمثلة على الاستخدام
- 🛠 استكشاف الأخطاء
- 📚 مراجع إضافية
