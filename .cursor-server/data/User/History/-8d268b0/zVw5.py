...
def run(path):
    ...
    try:
        ...
        if not os.path.exists(path):
            ...
            # إذا كان المسار يبدأ بـ /app/، نحاول تحويله للمسار الصحيح
            if path.startswith('/app/'):
                # /app/ هو المسار داخل Docker container
                # نحاول البحث في المسارات الممكنة
                relative_path = path.replace('/app/', '')
                current_dir = os.getcwd()
                alt_paths = [
                    os.path.abspath(os.path.join(current_dir, relative_path)),  # من المجلد الحالي
                    os.path.abspath(os.path.join(current_dir, 'app', relative_path)),  # من app/
                    os.path.join('/home/ai/ai-agent/backend', relative_path),  # من backend/
                    os.path.join('/home/ai/ai-agent/backend/app', relative_path),  # من backend/app/
                ]
                ...
                for alt_path in alt_paths:
                    ...
                    return f"[Errno 2] No such file or directory: '{path}'\n💡 Tip: Current directory is '{current_dir}'. Try:\n- Relative path from current directory\n- Absolute path like: /home/ai/ai-agent/backend/app/main.py"
            
            # إذا كان المسار يبدأ بـ /home/ai/ai-agent/، نحاول تحويله
            elif path.startswith('/home/ai/ai-agent/'):
                # نحاول البحث في المسارات الممكنة:
