    ...
    if (typeof window !== "undefined") {
      ...
      const checkBackendHealth = async () => {
        try {
          ...
          if (!response.ok) {
            console.warn(`[Health Check] Backend health check failed: ${response.status}`);
          } else {
            console.log(`[Health Check] Backend is healthy at ${apiUrl}`);
          }
        } catch (error) {
          if (error instanceof Error && error.name === 'AbortError') {
            console.warn(`[Health Check] Backend health check timed out at ${getApiUrl()}`);
          } else {
            console.error(`[Health Check] Cannot reach backend at ${getApiUrl()}:`, error);
          }
        }
      };
      checkBackendHealth();
  ...
  useEffect(() => {
    const fetchDashboardStats = async () => {
      try {
        ...
        try {
          const incidentsData = await apiRequest("/api/incidents/stats/today", { method: "GET" }, 10000).catch(() => null);
          if (incidentsData && typeof incidentsData.resolved_today === 'number') {
        ...
        // Fetch threats detected (24h)
        try {
          const threatsData = await apiRequest("/api/security/threat-detection/summary?hours=24", { method: "GET" }, 10000).catch(() => null);
          if (threatsData && typeof threatsData.total_threats === 'number') {
        ...
        // Fetch auto-hardening status
        try {
          const hardeningData = await apiRequest("/api/security/hardening/status", { method: "GET" }, 10000).catch(() => null);
          if (hardeningData && typeof hardeningData.actions_today === 'number') {
            setHardeningResults(hardeningData.actions_today);
          }
        } catch (e) {
          // Ignore errors, keep default value
        }
        ...
        try {
          const behaviorData = await apiRequest("/api/alerts/behavior/?resolved=false&hours=24", { method: "GET" }, 10000).catch(() => null);
          if (behaviorData && typeof behaviorData.count === 'number') {
  ...
  const sendQuery = async () => {
    ...
    try {
      ...
      if (errorMessage.includes("timeout")) {
        updateLastAgentMessage("⏱️ استغرق الرد وقتاً طويلاً جداً. قد يكون الـ Agent مشغول أو Ollama غير متاح.");
      } else if (errorMessage.includes("Failed to fetch") || errorMessage.includes("خطأ في الاتصال")) {
        updateLastAgentMessage(`🌐 ${errorMessage}\n\nحاول:\n1. التحقق من أن الـ Backend يعمل\n2. إعادة تحميل الصفحة\n3. التحقق من console للأخطاء`);
      } else if (errorMessage.includes("CORS")) {
        updateLastAgentMessage("🚫 خطأ CORS. تحقق من إعدادات الـ Backend.");
      } else {
