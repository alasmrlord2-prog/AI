...
export default function Home() {
  ...
  useEffect(() => {
    const fetchDashboardStats = async () => {
      // Fetch incidents today - increased timeout to 15s
      try {
        const incidentsData = await apiRequest("/api/incidents/stats/today", { method: "GET" }, 15000);
      ...
      // Fetch threats detected (24h) - increased timeout to 15s
      try {
        const threatsData = await apiRequest("/api/security/threat-detection/summary?hours=24", { method: "GET" }, 15000);
      ...
      // Fetch auto-hardening status - increased timeout to 15s
      try {
        const hardeningData = await apiRequest("/api/security/hardening/status", { method: "GET" }, 15000);
      ...
      // Fetch behavior anomalies - increased timeout to 15s
      try {
        const behaviorData = await apiRequest("/api/alerts/behavior/?resolved=false&hours=24", { method: "GET" }, 15000);
