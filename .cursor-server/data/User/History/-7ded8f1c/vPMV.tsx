...
export default function Home() {
  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      try {
        const data = await apiRequest("/api/monitor", { method: "GET" }, 30000);
  ...
  useEffect(() => {
    const fetchDashboardStats = async () => {
      // Fetch incidents today - increased timeout to 30s
      try {
        const incidentsData = await apiRequest("/api/incidents/stats/today", { method: "GET" }, 30000);
        if (incidentsData && typeof incidentsData === 'object' && typeof incidentsData.resolved_today === 'number') {
          setIncidentsToday(incidentsData.resolved_today);
        }
      } catch (e) {
        // Silently ignore - endpoint may require auth or may not exist
        if (process.env.NODE_ENV === 'development') {
          console.debug('[Dashboard] Failed to fetch incidents stats:', e instanceof Error ? e.message : String(e));
        }
      }

      // Fetch threats detected (24h) - increased timeout to 30s
      try {
        const threatsData = await apiRequest("/api/security/threat-detection/summary?hours=24", { method: "GET" }, 30000);
        if (threatsData && typeof threatsData === 'object' && typeof threatsData.total_threats === 'number') {
          setThreatsDetected(threatsData.total_threats);
        }
      } catch (e) {
        // Silently ignore - endpoint may require auth or may not exist
        if (process.env.NODE_ENV === 'development') {
          console.debug('[Dashboard] Failed to fetch threats data:', e instanceof Error ? e.message : String(e));
        }
      }

      // Fetch auto-hardening status - increased timeout to 30s
      try {
        const hardeningData = await apiRequest("/api/security/hardening/status", { method: "GET" }, 30000);
        if (hardeningData && typeof hardeningData === 'object') {
          // Handle different response structures
          const actionsToday = hardeningData.actions_today || hardeningData.actions || 0;
        ...
        // Silently ignore - endpoint may require auth or may not exist
        if (process.env.NODE_ENV === 'development') {
          console.debug('[Dashboard] Failed to fetch hardening data:', e instanceof Error ? e.message : String(e));
        }
      }

      // Fetch behavior anomalies - increased timeout to 30s
      try {
        const behaviorData = await apiRequest("/api/alerts/behavior/?resolved=false&hours=24", { method: "GET" }, 30000);
        if (behaviorData && typeof behaviorData === 'object' && typeof behaviorData.count === 'number') {
          setBehaviorAnomalies(behaviorData.count);
        }
      } catch (e) {
        // Silently ignore - endpoint may require auth or may not exist
        if (process.env.NODE_ENV === 'development') {
          console.debug('[Dashboard] Failed to fetch behavior data:', e instanceof Error ? e.message : String(e));
        }
      }

      // Fetch dashboard stats (modules, AI status, security health) - increased timeout to 30s
      try {
        const dashboardStats = await apiRequest("/api/dashboard/stats", { method: "GET" }, 30000);
        if (dashboardStats && typeof dashboardStats === 'object') {
          if (typeof dashboardStats.total_modules === 'number') {
            setTotalModules(dashboardStats.total_modules);
          }
          if (typeof dashboardStats.ai_status === 'string') {
            setAiStatus(dashboardStats.ai_status);
          }
          if (typeof dashboardStats.ai_status_text === 'string') {
            setAiStatusText(dashboardStats.ai_status_text);
          }
          if (typeof dashboardStats.security_health === 'number') {
            setSecurityHealth(dashboardStats.security_health);
          }
        }
      } catch (e) {
        // Silently ignore - endpoint may require auth or may not exist
        if (process.env.NODE_ENV === 'development') {
          console.debug('[Dashboard] Failed to fetch dashboard stats:', e instanceof Error ? e.message : String(e));
        }
      }
    };
              ...
              <div className="grid grid-cols-2 gap-3 md:gap-4">
                ...
                <Card className="bg-sw-bg-card border-sw-border shadow-sw-card dark:shadow-sw-card-dark">
                  <CardContent className="p-3 md:p-4">
                    ...
                    <div className="text-2xl md:text-3xl font-bold mt-1 md:mt-2 text-sw-success">{securityHealth.toFixed(1) || "0"}%</div>
