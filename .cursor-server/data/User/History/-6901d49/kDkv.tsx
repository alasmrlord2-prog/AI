...
export default function MonitoringPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [metricsRes, alertsRes, containersRes] = await Promise.all([
        fetch(`${API_URL}/api/monitor/server-metrics`),
        fetch(`${API_URL}/api/monitor/alerts?limit=10&unread_only=true`),
        fetch(`${API_URL}/api/monitor/containers`)
      ]);
      ...
      if (containersRes.ok) {
        ...
        const containersList = Object.values(data.containers || {});
  ...
  const handleAutoRepairService = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/monitor/auto-repair/service/${serviceName}`, {
        method: "POST"
      });

      if (res.ok) {
        const data = await res.json();
        alert(data.success ? "✅ تم إصلاح الخدمة!" : `⚠️ ${data.message}`);
        fetchData();
      } else {
        const data = await res.json();
        alert(data.detail || "فشل الإصلاح");
      }
    } catch (err: any) {
      alert(err.message);
    }
  ...
  const handleAutoRepairContainer = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/monitor/auto-repair/container/${containerName}`, {
        method: "POST"
      });

      if (res.ok) {
        const data = await res.json();
        alert(data.success ? "✅ تم إصلاح Container!" : `⚠️ ${data.error}`);
        fetchData();
      } else {
        const data = await res.json();
        alert(data.detail || "فشل الإصلاح");
  ...
  const handleCheckService = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/monitor/service-status/${serviceName}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const handleCheckContainer = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/monitor/container-status/${containerName}`);
      if (res.ok) {
        const data = await res.json();
