...
export default function MonitoringPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (!metrics && !alerts.length && !containers.length) {
        setLoading(true);
      }
      const [metricsData, alertsData, containersData] = await Promise.all([
        apiRequest("/api/monitor/server-metrics", {}, 3000).catch(() => null), // Faster timeout for dashboard
        apiRequest("/api/monitor/alerts?limit=10&unread_only=true", {}, 3000).catch(() => ({ alerts: [] })),
        apiRequest("/api/monitor/containers", {}, 3000).catch(() => ({ containers: {} }))
      ]);
  ...
  const handleCheckService = async () => {
    ...
    try {
      const data = await apiRequest(`/api/monitor/service-status/${serviceName}`, {}, 5000);
  ...
  const handleCheckContainer = async () => {
    ...
    try {
      const data = await apiRequest(`/api/monitor/container-status/${containerName}`, {}, 5000);
