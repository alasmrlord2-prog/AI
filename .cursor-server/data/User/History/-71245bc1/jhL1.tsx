...
export default function SecurityCenterPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch threats
        const threatsData = await apiRequest("/api/threat-detection/alerts", { method: "GET" }, 5000).catch(() => ({ threats: [] }));
        setThreats(threatsData?.threats || []);

        // Fetch behavior anomalies
        const anomaliesData = await apiRequest("/api/behavior-alerts", { method: "GET" }, 5000).catch(() => ({ anomalies: [] }));
        setAnomalies(anomaliesData?.anomalies || []);

        // Fetch secrets
        const secretsData = await apiRequest("/api/secrets/list", { method: "GET" }, 5000).catch(() => ({ secrets: [] }));
        setSecrets(secretsData?.secrets || []);

        // Fetch hardening suggestions
        const hardeningData = await apiRequest("/api/hardening/suggestions", { method: "GET" }, 5000).catch(() => ({ suggestions: [] }));
        setHardening(hardeningData?.suggestions || []);
      } catch (error) {
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {threats.length > 0 ? (
                    threats.slice(0, 10).map((threat, idx) => (
                      <div key={idx} className="p-3 bg-slate-800 rounded border-l-4 border-red-500">
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {anomalies.length > 0 ? (
                    anomalies.slice(0, 10).map((anomaly, idx) => (
                      <div key={idx} className="p-3 bg-slate-800 rounded border-l-4 border-yellow-500">
                        <div className="flex justify-between items-start">
                          <div>
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {secrets.length > 0 ? (
                    secrets.filter((s: any) => s.exposed || s.risk === "high").slice(0, 10).map((secret, idx) => (
                      <div key={idx} className="p-3 bg-slate-800 rounded border-l-4 border-orange-500">
                        <div className="flex justify-between items-start">
                          <div>
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {hardening.length > 0 ? (
                    hardening.slice(0, 10).map((suggestion, idx) => (
                      <div key={idx} className="p-3 bg-slate-800 rounded border-l-4 border-blue-500">
                        <div className="flex justify-between items-start">
                          <div>
