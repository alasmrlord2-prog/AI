    ...
    <div className="flex h-screen bg-slate-950 text-slate-200">
      ...
      {result && (
        <div className="space-y-4">
              ...
              <CardContent>
                <div className="space-y-2">
                  <div className="text-sm">
                    ...
                    <span className={`${
                      result.reputation === "malicious" ? "text-red-400" : "text-green-400"
                    }`}>
                ...
                <div className="space-y-2">
                  ...
                  {result.top_connections.map((conn, idx) => (
                    <div key={idx} className="flex items-center justify-between border border-slate-700 rounded p-2">
                      <div className="font-mono text-sm text-slate-300">{conn[0]}</div>
                      <div className="text-sm text-blue-400">{conn[1]} connections</div>
                    </div>
                  ))}
                </div>
              </CardContent>
          ...
          {result.cve_database && result.cve_database.length > 0 && (
            <Card className="bg-slate-900 border-slate-800">
              <CardHeader>
              ...
              <CardContent>
                <div className="space-y-2">
                  {result.cve_database.map((cve, idx) => (
                    <div key={idx} className={`p-3 border rounded ${getRiskColor(cve.risk)}`}>
                      <div className="font-semibold">{cve.service}</div>
                      <div className="text-xs text-slate-400">CVE: {cve.cve}</div>
                    </div>
                  ))}
                </div>
              </CardContent>
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  {result.s3_buckets.map((bucket, idx) => (
                    <div key={idx} className="text-sm text-slate-300 font-mono">{bucket}</div>
                  ))}
                </div>
              </CardContent>
            </Card>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    {siemData?.metrics?.events_by_source && Object.keys(siemData.metrics.events_by_source).length > 0 && (
                      <Card className="bg-slate-900 border-slate-800">
                        ...
                        <CardContent>
                          <BarChart
                            data={Object.entries(siemData.metrics.events_by_source)
                              .sort((a, b) => b[1] - a[1])
                              .slice(0, 8)
