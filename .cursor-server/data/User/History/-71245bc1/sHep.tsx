  ...
  const runScan = async () => {
    ...
    try {
      ...
      if (data.error && typeof data.error === 'object' && data.error.status === 'pending') {
        ...
        setResult({ ...data, error: `⏳ Action requires approval. Action ID: ${data.error.action_id || 'N/A'}\n${data.error.message || 'Please approve this action in the Approvals page.'}` });
      } else {
        setResult(data);
      }
    } catch (err) {
          ...
          {activeTab === "siem" && (
            <div className="space-y-6">
              ...
              {siemLoading && !siemData ? (
                ...
                <>
                  {/* Generate Mock Data for Layered Analytics */}
                  {(() => {
                    const now = new Date();
                    const caseTimeline = [];
                    for (let i = 6; i >= 0; i--) {
                    ...
                    const highRiskEntities = Object.values(entities).reduce((sum, entity) => sum + entity.high_risk, 0);
