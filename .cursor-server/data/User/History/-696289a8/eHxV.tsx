...
export default function AuditPage() {
  ...
  const fetchLogs = useCallback(async () => {
    try {
      ...
      const data = await apiRequest(url, {}, 5000);
      setLogs(data.logs || []);
  ...
  const handleExport = async () => {
    try {
      const data = await apiRequest("/api/audit/export?output_file=audit_export.json", {}, 10000);
      alert(`تم التصدير بنجاح: ${data.file || "audit_export.json"}`);
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : "خطأ غير معروف";
      alert(`خطأ في التصدير: ${errorMsg}`);
    }
  };
                      ...
                      <div key={log.id} className="p-4 bg-gradient-to-r from-slate-50 to-slate-100 rounded-lg border border-slate-200 hover:shadow-md transition-shadow">
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            ...
                            {(log.resource_id || log.feature_key) && (
                              <div className="text-xs text-slate-500 mt-1">
                                {log.resource_id && (
                                  <span className="mr-2">Resource ID: {log.resource_id}</span>
                                )}
                                {log.feature_key && (
                                  <span>Feature: {log.feature_key}</span>
                                )}
                              </div>
                            )}
                            {log.payload && (
