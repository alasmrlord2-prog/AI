...
export default function MonitorPage() {
  ...
  const fetchStatus = async () => {
    try {
      const apiUrl = `${API_URL}/api/monitor`;
      ...
      try {
        ...
        const errorMessage = fetchErr.name === 'AbortError' 
          ? `Request timeout. Backend may not be responding on ${API_URL}`
          : fetchErr.message || 'Unknown network error';
        setError(`Network error: ${errorMessage}. Make sure backend is running on ${API_URL}`);
        setData(null);
        setLoading(false);
        return;
      }
      ...
      if (!res) {
        setError(`No response received. Make sure backend is running on ${API_URL}`);
        setData(null);
        setLoading(false);
        return;
      }
