...
export default function MonitorPage() {
  ...
  const fetchStatus = async () => {
    ...
    try {
      // Very short timeout - if backend is slow, page still works
      // Data will load in background on next interval
      ...
      // Only show error if we don't have any previous data (first load)
      // If we have data, keep showing it and just log the error
      if (!data) {
        if (errorMessage.includes("timeout")) {
          setError("⏱️ استغرق الطلب وقتاً طويلاً. تحقق من حالة الـ Backend. سيتم المحاولة مرة أخرى تلقائياً...");
        } else if (errorMessage.includes("الاتصال") || errorMessage.includes("Failed to fetch")) {
          setError("🌐 خطأ في الاتصال بالخادم. تحقق من اتصال الإنترنت وحالة الـ Backend. سيتم المحاولة مرة أخرى تلقائياً...");
        } else {
          setError(`❌ خطأ: ${errorMessage}. سيتم المحاولة مرة أخرى تلقائياً...`);
        }
      } else {
        // If we have previous data, just log the error but don't show it to user
        // This prevents the UI from flickering when there are temporary network issues
        console.warn("Monitor data fetch failed, but keeping previous data:", errorMessage);
      }
    } finally {
      setLoading(false);
      fetchingRef.current = false;
  ...
  useEffect(() => {
    ...
    }, 10000); // 10 seconds - less frequent updates for better performance
