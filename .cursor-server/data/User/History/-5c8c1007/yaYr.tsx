...
export default function MonitorPage() {
  ...
  const fetchStatus = async () => {
    try {
      const apiUrl = `${getApiUrl()}/api/monitor`;
      console.log("Fetching monitor data from:", apiUrl);
      ...
      try {
        ...
        const errorMessage = fetchErr.name === 'AbortError' 
          ? `Request timeout. Backend may not be responding on ${getApiUrl()}`
          : fetchErr.message || 'Unknown network error';
        setError(`Network error: ${errorMessage}. Make sure backend is running on ${getApiUrl()}`);
        setData(null);
        setLoading(false);
        return;
      }
      ...
      if (!res) {
        setError(`No response received. Make sure backend is running on ${getApiUrl()}`);
        setData(null);
        setLoading(false);
        return;
      }
