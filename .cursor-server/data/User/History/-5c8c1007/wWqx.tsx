...
// Use same host as frontend if no env var is set
const getApiUrl = () => {
  if (typeof window !== "undefined") {
    // If running in browser, use same host with port 8000
    const host = window.location.hostname;
    return process.env.NEXT_PUBLIC_AGENT_API_URL || 
...
export default function MonitorPage() {
  ...
  const fetchStatus = async () => {
    try {
      console.log("Fetching monitor data from:", `${API_URL}/api/monitor`);
      const res = await fetch(`${API_URL}/api/monitor`, {
        ...
        setError(`Network error: ${err.message}. Make sure backend is running on ${API_URL}`);
        setData(null);
        setLoading(false);
        return null;
      });
      ...
      if (!res) {
        return; // Error already handled in catch
      }
