...
export default function PlatformPage() {
  ...
  const fetchData = async () => {
    try {
      if (tab === "tenants" || !tab) {
        // Tenant management API not available yet, using empty state
        // TODO: Add tenant management API endpoint
        setTenants([]);
      } else if (tab === "billing") {
              ...
              <Card className="bg-slate-900 border-slate-800">
                ...
                <CardContent>
                  <div className="space-y-3">
                    {tenants.length > 0 ? (
                      tenants.map((tenant) => (
                        <div key={`tenant-${tenant.id}-${tenant.name}`} className="p-3 md:p-4 bg-slate-800 rounded border-l-4 border-blue-500">
              ...
              <Card className="bg-slate-900 border-slate-800">
                ...
                <CardContent>
                  <div className="space-y-3">
                    {tenants.length > 0 ? (
                      tenants.map((tenant) => (
                        <div key={`usage-${tenant.id}-${tenant.name}`} className="p-3 md:p-4 bg-slate-800 rounded">
                  ...
                  <div className="space-y-2 max-h-96 overflow-y-auto">
                    {invoices.length > 0 ? (
                      invoices.map((invoice) => (
                        <div key={`invoice-${invoice.id}-${invoice.tenant_id}`} className="p-3 md:p-4 bg-slate-800 rounded border-l-4 border-blue-500">
                          <div className="flex justify-between items-start">
                            <div>
                  ...
                  <div className="space-y-2">
                    {paymentMethods.length > 0 ? (
                      paymentMethods.map((pm) => (
                        <div key={`pm-${pm.id}-${pm.type}`} className="p-3 md:p-4 bg-slate-800 rounded border-l-4 border-purple-500">
                          <div className="flex justify-between items-start">
                            <div>
