...
def run(svc):
    """
    فحص حالة أي خدمة على السيرفر مباشرة بناءً على الإعدادات.
    
    يعمل داخل Docker container والـ host على حد سواء.
    يستخدم طرق متعددة للفحص:
    1. systemctl (إذا متوفر)
    2. psutil للبحث في العمليات
    3. pgrep كبديل
    4. ps aux كحل أخير
    5. docker ps (إذا كان داخل container)
    """
    try:
        # تحميل الإعدادات (للمستقبل - يمكن إضافة قيود على الخدمات)
        ...
        # Check Docker containers if we're in a Docker environment
        try:
            docker_result = subprocess.run(
                ["docker", "ps", "--format", "{{.Names}}", "--filter", f"name={svc}"],
                capture_output=True,
                text=True,
                timeout=3
            )
                ...
                for container in containers:
                    if container:
                        ...
                        try:
                            ...
                            container_info.append(f"  Container: {container} (Status: {status})")
                        except:
                            container_info.append(f"  Container: {container}")
                
                if container_info:
                    return f"Service '{svc}' found in Docker containers:\n" + "\n".join(container_info)
        ...
        if PSUTIL_AVAILABLE:
            try:
                ...
                # خريطة أوسع لأسماء الخدمات الشائعة
                process_map = {
                    "ssh": "sshd",
                    "docker": "dockerd",
                    "nginx": "nginx",
