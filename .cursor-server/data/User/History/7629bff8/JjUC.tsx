...
export default function BillingPage() {
  ...
  const loadBillingData = async () => {
    try {
      ...
      // Load invoices
      const invoicesRes = await fetch(`${getApiUrl()}/api/billing/invoices`, {
      ...
      // Load subscriptions
      const subsRes = await fetch(`${getApiUrl()}/api/billing/subscriptions`, {
        headers: { "Authorization": `Bearer ${token}` },
      });
      if (subsRes.ok) {
      ...
      const pmRes = await fetch(`${getApiUrl()}/api/billing/payment-methods`, {
        headers: { "Authorization": `Bearer ${token}` },
      });
      if (pmRes.ok) {
  ...
  const handlePayInvoice = async (invoiceId: string) => {
    ...
    try {
      const token = getAuthToken();
      const res = await fetch(`${getApiUrl()}/api/billing/invoices/${invoiceId}/pay`, {
        method: "POST",
        headers: { "Authorization": `Bearer ${token}` },
      });
      if (res.ok) {
  ...
  const handleSetDefault = async (paymentMethodId: string) => {
    ...
    try {
      const token = getAuthToken();
      const res = await fetch(`${getApiUrl()}/api/billing/payment-methods/${paymentMethodId}/set-default`, {
        method: "POST",
        headers: { "Authorization": `Bearer ${token}` },
      });
      if (res.ok) {
