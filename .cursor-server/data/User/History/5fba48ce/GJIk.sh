...
for i in {1..30}; do
    ...
    if [ $i -eq 30 ]; then
        print_warning "Backend is taking longer than expected, but continuing..."
    fi
    sleep 1
done
...
# Fix Prometheus config with correct IP
echo ""
echo "Configuring Prometheus..."
SERVER_IP=$(hostname -I | awk '{print $1}')
if [ -f "prometheus/prometheus.yml" ]; then
    sed -i "s|host.docker.internal:8000|${SERVER_IP}:8000|g" prometheus/prometheus.yml
    sed -i "s|172.31.20.228:8000|${SERVER_IP}:8000|g" prometheus/prometheus.yml
    print_status "Prometheus config updated with IP: $SERVER_IP"
    
    # Reload Prometheus config
    sleep 2
    curl -X POST http://localhost:9090/-/reload 2>/dev/null && print_status "Prometheus config reloaded" || print_warning "Prometheus reload failed (will use on restart)"
fi
