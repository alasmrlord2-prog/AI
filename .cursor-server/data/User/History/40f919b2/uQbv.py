...
async def get_alerts(
    ...
    try:
        ...
        # Use timeout to prevent hanging (max 5 seconds)
        loop = asyncio.get_event_loop()
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(lambda: get_behavior_alerter().get_alerts(severity=severity, hours=hours, resolved=resolved))
        ...
        # Return default response if alerter fails
        return {"alerts": [], "count": 0}
