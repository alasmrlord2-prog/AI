...
نظام شامل يتكون من **ثلاث واجهات منفصلة تماماً**:
...
- نسخ `nginx-dashboard.conf` → Dashboard (port 3000)
- نسخ `nginx-crm.conf` → CRM (port 3001)
- نسخ `nginx-aaa.conf` → AAA (port 3002)
- تفعيل المواقع في `/etc/nginx/sites-available/`
- اختبار التكوين
- إعادة تحميل NGINX

**للمزيد من التفاصيل عن إعداد NGINX، راجع: `NGINX_SETUP.md`**
...
هذا سيشغل:
- **Dashboard** على port 3000 (ai-agent.bankid-sy.com)
- **CRM** على port 3001 (crm.bankid-sy.com)
- **AAA** على port 3002 (aaa.bankid-sy.com)
...
### Frontend URLs:
- **Dashboard:** http://ai-agent.bankid-sy.com (port 3000) - واجهة المستخدم العادي
- **CRM:** http://crm.bankid-sy.com (port 3001) - واجهة إدارة Tenants/Users/Subscriptions
- **AAA:** http://aaa.bankid-sy.com (port 3002) - واجهة إدارة Authentication/Authorization/Accounting
...
- **NGINX_SETUP.md** - دليل إعداد NGINX الكامل
- **PROJECT_STRUCTURE.md** - بنية المشروع الكاملة
