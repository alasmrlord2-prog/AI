...
```json
{
  ...
  "host_mount_path": "/host/ai-agent"  // مسار mount المجلد الرئيسي من host
}
```
...
- **`host_mount_path`**: مسار mount المجلد الرئيسي من host داخل container (افتراضي: `/host/ai-agent`).

### الوصول للملفات:
- **داخل Container**: المسار الحالي هو `/app` (مجلد backend)
- **من Host**: المجلد الرئيسي `/home/ai/ai-agent/` مربوط في `/host/ai-agent/` داخل container
- **تحويل تلقائي**: عند محاولة قراءة `/home/ai/ai-agent/file.txt`، يتم البحث تلقائياً في `/host/ai-agent/file.txt`

### أمثلة:
- قراءة ملف من `/app/README.md` (داخل container - مجلد backend)
- قراءة ملف من `/host/ai-agent/docker-compose.yml` (من host)
- قراءة ملف من `/home/ai/ai-agent/docker-compose.yml` (يتم تحويله تلقائياً إلى `/host/ai-agent/docker-compose.yml`)
- قراءة ملف من `/etc/nginx/nginx.conf` (إذا كان مسموح ومتاح)
