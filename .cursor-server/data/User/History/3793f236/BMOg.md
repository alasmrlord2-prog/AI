...
### 1. إعداد SSL Certificate (Let's Encrypt)

```bash
cd /home/ai/ai-agent

# تثبيت شهادة SSL من Let's Encrypt
./install-ssl.sh
```

**ملاحظة**: قبل تشغيل السكربت:
- تأكد من أن nginx يعمل
- تأكد من أن الدومين `ai-agent.bankid-sy.com` يشير إلى IP `3.76.209.35`
- في Cloudflare، أزل Proxy (DNS Only) أو غيّر SSL/TLS mode إلى "Full"
...
الشهادة ستنتهي بعد 90 يوم. يمكن إضافة cron job للتجديد التلقائي:

```bash
# إضافة إلى crontab
0 0 * * * cd /home/ai/ai-agent && ./renew-ssl.sh
```
