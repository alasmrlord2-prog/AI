...
# Use the shared path resolver from utils
...
def scan_docker_security() -> Dict[str, Any]:
    ...
    try:
        ...
        # Also check /home and /opt if they exist (system paths)
        ...
        # Use same search paths as Dockerfiles
        search_paths = []
        for path_str in ["/app", "app", "./app", "."]:
            try:
                resolved = resolve_path(path_str)
...
def scan_vulnerabilities() -> Dict[str, Any]:
    ...
    try:
        ...
        # Check for outdated packages (apt)
        try:
        ...
        # Check Python packages (pip audit)
        try:
            ...
            if pip_audit_result.returncode == 0:
                try:
                    ...
                    for vuln in audit_data.get("vulns", []):
            ...
            # Fallback: check requirements.txt and check for known vulnerable packages
            try:
                        ...
                        with open(req_file, "r") as f:
                            for line in f:
                                if "==" in line:
                                    ...
                                    # Known vulnerable packages list (can be expanded)
                                    vulnerable_pkgs = ["django", "flask", "requests"]
                                    if any(vp in pkg.lower() for vp in vulnerable_pkgs):
                                        results["python_packages"].append({
                                            "package": pkg,
                                            "status": "needs_audit",
                                            "risk": "medium",
        ...
        try:
            # Look for package.json files - use resolve_path
            project_root = resolve_path(".")
...
def scan_malware(path: str = "/tmp") -> Dict[str, Any]:
    ...
    try:
        ...
        # Known malware signatures (simple hash-based - can be expanded)
        malware_signatures = set()  # Can be populated from external source
        ...
        if tmp_path.exists():
            for file_path in tmp_path.rglob("*"):
                if file_path.is_file():
                    ...
                    # Check file hash (if file is small enough)
                    try:
                        if file_path.stat().st_size < 10 * 1024 * 1024:  # 10MB limit
                            ...
                            if header[:4] == b'\x7fELF':  # ELF binary
                                if file_name.endswith(('.exe', '.dll', '.bat')):
                                    is_suspicious = True
                                    reasons.append("ELF binary with suspicious extension")
                            elif header[:2] == b'MZ':  # PE/Windows binary
                                if not file_name.endswith(('.exe', '.dll', '.sys')):
                                    is_suspicious = True
                                    reasons.append("PE binary with unusual extension")
                    except:
                        pass
...
def scan_intrusion_detection() -> Dict[str, Any]:
    ...
    try:
        ...
        # Check auth.log for sudo failures and unusual activity
        try:
            ...
            for auth_log in auth_logs:
                if auth_log.exists():
                    ...
                    for line in lines:
                        ...
                        # Detect sudo failures
                        if "sudo" in line_lower and ("failed" in line_lower or "incorrect" in line_lower):
                            # Extract user
                            user_match = re.search(r'user=(\w+)', line)
                            if user_match:
                                user = user_match.group(1)
                                sudo_failures[user] = sudo_failures.get(user, 0) + 1
                        
                        # Detect sudo successes (unusual patterns)
                        if "sudo" in line_lower and "successful" in line_lower:
                            user_match = re.search(r'user=(\w+)', line)
                            if user_match:
                                user = user_match.group(1)
                                sudo_successes[user] = sudo_successes.get(user, 0) + 1
                    
                    # Flag users with many sudo failures
                    for user, count in sudo_failures.items():
                        if count > 5:
                            results["intrusions"].append({
                                "type": "sudo_failure_anomaly",
                                "severity": "high",
                                "message": f"User {user} has {count} sudo failures",
                                "user": user,
                                "count": count,
                            })
                            results["summary"]["intrusions_detected"] += 1
                    
                    # Flag unusual sudo usage
                    for user, count in sudo_successes.items():
                        if count > 20:  # High sudo usage
                            results["anomalies"].append({
                                "type": "excessive_sudo_usage",
                                "severity": "medium",
                                "message": f"User {user} has {count} sudo commands (unusual pattern)",
                                "user": user,
                                "count": count,
                            })
        ...
        try:
            ...
            if cron_result.returncode == 0:
                cron_lines = cron_result.stdout.strip().split("\n")
                suspicious_crons = []
                for line in cron_lines:
                    if line.strip() and not line.startswith("#"):
                        # Check for suspicious patterns
                        if any(pattern in line.lower() for pattern in ["curl", "wget", "bash -c", "sh -c"]):
                            suspicious_crons.append(line)
                
                if suspicious_crons:
                    results["intrusions"].append({
                        "type": "suspicious_cron",
                        "severity": "high",
                        "message": "Suspicious cron jobs detected",
        ...
        # Note: /home is a system directory, but we need to check it for SSH keys
        # This is a security check, so we allow it (not using resolve_path for system paths)
        try:
            # Check if /home exists (system path)
                ...
                for user_dir in ssh_dir.iterdir():
                    if user_dir.is_dir():
                        ...
                        if authorized_keys.exists():
                            # Check modification time (recent changes)
