...
export default function WorkflowsPage() {
  ...
  const fetchWorkflows = useCallback(async () => {
    try {
      setLoading(true);
      const data = await apiRequest("/api/workflows", {}, 5000);
  ...
  const handleExecute = async (workflowId: string) => {
    try {
      ...
      const data = await apiRequest(`/api/workflows/${workflowId}/execute`, {
        method: "POST",
      }, 30000); // 30s timeout for workflow execution
  ...
  const fetchExecutions = async (workflowId: string) => {
    try {
      const data = await apiRequest(`/api/workflows/executions?workflow_id=${workflowId}&limit=10`, {}, 5000);
                    ...
                    {executionResult.steps && executionResult.steps.length > 0 && (
                      <div>
                        ...
                        <div className="mt-2 space-y-1">
                          {Array.isArray(executionResult.steps) ? executionResult.steps.map((step: Record<string, unknown>, idx: number) => {
                            const stepObj = step as { node_type?: string; result?: { output?: unknown } };
                            return (
                              <div key={idx} className="text-xs bg-white p-2 rounded">
                                {stepObj.node_type}: {stepObj.result?.output ? JSON.stringify(stepObj.result.output) : "OK"}
                              </div>
                            );
                          }) : null}
                        </div>
