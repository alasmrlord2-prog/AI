...
export default function DebuggerPage() {
  ...
  const startWatching = async () => {
    try {
      await apiRequest("/api/debugger/watch/start", {
        method: "POST",
  ...
  const stopWatching = async () => {
    try {
      await apiRequest("/api/debugger/watch/stop", {
        method: "POST"
      }, 5000);
      setWatching(false);
    } catch (err) {
  ...
  const toggleAutoFix = async () => {
    try {
      ...
      await apiRequest(endpoint, { method: "POST" }, 5000);
      setAutoFix(!autoFix);
    } catch (err) {
  ...
  const handleAnalyzeError = async (error: DebugError) => {
    try {
      const data = await apiRequest("/api/debugger/analyze", {
        method: "POST",
      ...
      alert(`تحليل الخطأ:\n\nRoot Cause: ${data.root_cause || "غير محدد"}\n\nSolution: ${data.solution || "غير محدد"}\n\nSafe to Fix: ${data.safe_to_fix ? "Yes" : "No"}`);
    } catch (err) {
