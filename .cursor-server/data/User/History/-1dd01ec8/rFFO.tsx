...
type PaymentMethod = {
  id: string;
  type: "card" | "bank" | "paypal" | "manual" | "electronic" | "bank_transfer" | "sham_cash";
  last4?: string;
  brand?: string;
  expiry_month?: number;
  expiry_year?: number;
  ...
  bank_name?: string;
  provider?: string; // اسم مزود الدفع الإلكتروني
};
...
export default function BillingPage() {
  ...
  const loadBillingData = async () => {
    try {
      // Load invoices
      const invoicesData = await apiRequest("/api/billing/invoices", {}, 5000);
      setInvoices(invoicesData.invoices || []);

      // Load subscriptions
      const subsData = await apiRequest("/api/billing/subscriptions", {}, 5000);
      setSubscriptions(subsData.subscriptions || []);

      // Load payment methods
      const pmData = await apiRequest("/api/billing/payment-methods", {}, 5000);
  ...
  const handlePayInvoice = async (invoiceId: string) => {
    setLoading(true);
    try {
      await apiRequest(`/api/billing/invoices/${invoiceId}/pay`, {
        method: "POST",
      }, 10000);
      ...
      alert(`Failed to pay invoice: ${errorMsg}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSetDefault = async (paymentMethodId: string) => {
    setLoading(true);
    try {
      await apiRequest(`/api/billing/payment-methods/${paymentMethodId}/set-default`, {
        method: "POST",
      }, 5000);
      await loadBillingData();
      ...
      alert(`Failed to set default payment method: ${errorMsg}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEditPaymentMethod = (paymentMethod: PaymentMethod) => {
    // For now, show an alert. You can implement a modal or form later
    alert(`Edit payment method: ${paymentMethod.type} ending in ${paymentMethod.last4}\n\nThis feature will open an edit form.`);
  };

  const handleAddPaymentMethod = () => {
    // For now, show an alert. You can implement a modal or form later
    alert("Add Payment Method\n\nThis feature will open a form to add a new payment method.");
  };
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="flex gap-2 mb-6 border-b border-slate-800">
            {[
              ...
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id as "overview" | "invoices" | "subscriptions" | "payment")}
                    ...
                    {paymentMethods.length === 0 ? (
                      ...
                      paymentMethods.map((pm) => (
                        ...
                        >
                          <div className="flex items-center gap-4">
                            <div className="text-2xl">
                              {pm.type === "card" ? "💳" : 
                               ...
                               pm.type === "manual" ? "✋" :
                               pm.type === "electronic" ? "⚡" :
                               pm.type === "bank_transfer" ? "🏛️" :
                               pm.type === "sham_cash" ? "💵" : "💳"}
                            </div>
                            <div>
                              <div className="font-bold text-slate-300 capitalize">
                                {pm.type === "card" ? `Card ${pm.brand ? `(${pm.brand})` : ""}` :
                                 pm.type === "manual" ? "دفع يدوي" :
                                 pm.type === "electronic" ? `دفع إلكتروني ${pm.provider ? `(${pm.provider})` : ""}` :
                                 pm.type === "bank_transfer" ? `تحويل بنكي ${pm.bank_name ? `(${pm.bank_name})` : ""}` :
                                 pm.type === "sham_cash" ? "شام كاش" :
                                 pm.type === "bank" ? "Bank Account" :
                              ...
                              {pm.bank_name && pm.type !== "bank_transfer" && (
                                <div className="text-sm text-slate-400">
                                  Bank: {pm.bank_name}
                                </div>
                              )}
                              {pm.provider && pm.type === "electronic" && (
                                <div className="text-sm text-slate-400">
                          ...
                          <Button 
                            onClick={() => pm.is_default ? handleEditPaymentMethod(pm) : handleSetDefault(pm.id)}
                            disabled={loading}
                            className="bg-slate-700 hover:bg-slate-600"
                          >
