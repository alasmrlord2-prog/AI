...
export default function Sidebar() {
    ...
    if (hasChildren) {
      return (
        <div key={uniqueKey} className="space-y-0.5">
          ...
          {isExpanded && (
            <div className="ml-2 space-y-0.5 mt-0.5 border-l border-slate-700/50 pl-2">
  ...
  return (
    <aside
      className="w-64 bg-slate-900 h-screen border-r border-slate-800/50 hidden md:flex flex-col overflow-hidden"
        ...
        <div className="space-y-6">
          {menuGroups.map((group) => (
            <div key={group.title} className="space-y-1.5">
              <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-wider px-3 py-1.5">
                {group.title}
              </h3>
