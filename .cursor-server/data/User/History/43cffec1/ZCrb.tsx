        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <Card className="bg-slate-900 border-slate-800">
              <CardHeader>
                <CardTitle>Events Trend (Last Hour)</CardTitle>
              </CardHeader>
              <CardContent>
                <LineChart
            ...
            <Card className="bg-slate-900 border-slate-800">
              ...
              <CardContent>
                {data?.metrics && (
                  <PieChart
                    data={[
                      { label: "Critical", value: data.metrics.critical_severity || 0, color: "#ef4444" },
                      { label: "High", value: data.metrics.high_severity || 0, color: "#f59e0b" },
                      { label: "Medium", value: data.metrics.medium_severity || 0, color: "#eab308" },
                      { label: "Low", value: data.metrics.low_severity || 0, color: "#10b981" },
                    ].filter(d => d.value > 0)}
                    size={200}
                  />
                )}
            ...
            {data?.metrics?.events_by_source && Object.keys(data.metrics.events_by_source).length > 0 && (
              <Card className="bg-slate-900 border-slate-800">
                ...
                <CardContent>
                  <BarChart
                    data={Object.entries(data.metrics.events_by_source)
                      .sort((a, b) => b[1] - a[1])
                      .slice(0, 8)
            ...
            {data?.metrics?.events_by_type && Object.keys(data.metrics.events_by_type).length > 0 && (
              <Card className="bg-slate-900 border-slate-800">
                ...
                <CardContent>
                  <BarChart
                    data={Object.entries(data.metrics.events_by_type)
                      .sort((a, b) => b[1] - a[1])
                      .slice(0, 8)
