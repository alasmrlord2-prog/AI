    ...
    try {
      ...
      if (json.events && json.events.length > 0) {
        const newAlerts: Alert[] = json.events
          .filter((e: { severity?: string }) => e.severity === "critical" || e.severity === "high")
          .slice(0, 5)
          .map((e: { severity?: string; type?: string; message?: string; timestamp?: string; source?: string }) => ({
            id: `alert-${Date.now()}-${Math.random()}`,
            type: (e.severity?.toLowerCase() || "info") as "critical" | "high" | "medium" | "low" | "info",
      ...
      if (json.metrics && typeof json.metrics.total_events === 'number') {
        const now = new Date();
        const timeLabel = `${now.getHours()}:${now.getMinutes().toString().padStart(2, '0')}`;
        setEventHistory(prev => {
          const newData = { label: timeLabel, value: json.metrics.total_events };
    ...
    return (
      <div className="flex h-screen w-screen overflow-hidden bg-slate-950 text-slate-200">
        ...
        <div className="flex flex-col flex-1">
          <Header />
          <main className="flex-1 overflow-y-auto overflow-x-hidden p-6 flex items-center justify-center scrollbar-thin">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <Card className="bg-gradient-to-br from-slate-900 to-slate-800 border-slate-700">
              ...
              <CardContent>
                <div className="h-64">
                  <LineChart
                    data={mockData.caseTimeline.map((d) => ({
              ...
              <CardContent>
                <div className="flex items-center justify-center">
                  <div className="relative">
                    <PieChart
                      data={[
                        { label: "4-Catastrophic", value: mockData.cases.catastrophic, color: "var(--sw-teal)" },
                        { label: "3-Critical", value: mockData.cases.critical, color: "var(--sw-primary-dark)" },
                        { label: "2-Marginal", value: mockData.cases.marginal, color: "var(--sw-secondary-light)" },
                        { label: "1-Insignificant", value: mockData.cases.insignificant, color: "var(--sw-blue)" },
                        { label: "0-None", value: mockData.cases.none, color: "var(--sw-border)" },
                      ]}
                      size={200}
                    />
