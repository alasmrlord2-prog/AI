...
async def get_today_incidents_stats(
    ...
    try:
        ...
        def query_db():
            ...
            row = cursor.fetchone()
            total_today = row[0] if row and row[0] is not None else 0
            resolved_today = row[1] if row and row[1] is not None else 0
        ...
        # Execute with aggressive timeout protection (max 2 seconds)
        ...
        print("Timeout getting today's stats - using defaults")
        return {
            "total_today": 0,
            "resolved_today": 0,
            "open_today": 0
