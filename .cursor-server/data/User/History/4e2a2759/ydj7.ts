...
export const getApiUrl = (): string => {
  ...
  // Client-side: For browser requests, use relative URLs to leverage Next.js rewrites
  ...
  // Check if we should use direct backend URL (for WebSocket or specific cases)
  if (process.env.NEXT_PUBLIC_AGENT_API_URL) {
    const envUrl = process.env.NEXT_PUBLIC_AGENT_API_URL;
    if (envUrl.startsWith('http://') || envUrl.startsWith('https://')) {
      // Only use direct URL if explicitly set and not using rewrites
      return envUrl;
    }
  }
  
  // For API requests, use relative URLs - Next.js rewrites will handle routing
  // This works for: localhost, IP addresses, and domains
  return '';
};
...
export const apiRequest = async (
  ...
  if (lastError) {
    ...
    // Log the error for debugging - simple and clear
