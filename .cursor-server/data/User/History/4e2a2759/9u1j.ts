...
export const apiRequest = async (
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (!isJson) {
        // If status is OK but not JSON, it might be an HTML error page
        if (response.status === 200) {
          const text = await response.text();
          console.warn('Response is not JSON but status is 200, received:', text.substring(0, 100));
          // Try to return empty object or handle gracefully
          return {};
        }
        const text = await response.text();
