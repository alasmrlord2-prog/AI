...
export const getApiUrl = (): string => {
  ...
  // If accessing from IP address (e.g., 3.76.209.35), use the same IP with port 8000
  // This handles cases where frontend is accessed via IP address
  ...
  // But if we're on a different port, try to use port 8000 on same hostname
  if (port && port !== '8000') {
    return `http://${hostname}:8000`;
  }
