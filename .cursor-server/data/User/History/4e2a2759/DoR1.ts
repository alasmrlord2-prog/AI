  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      try {
        ...
        // Log fetch error immediately for debugging
        if (process.env.NODE_ENV === 'development' || typeof window !== 'undefined') {
          console.error('[API] Fetch error caught:', fetchError);
          console.error('[API] Fetch error type:', fetchError instanceof Error ? fetchError.constructor.name : typeof fetchError);
          console.error('[API] Fetch error message:', fetchError instanceof Error ? fetchError.message : String(fetchError));
        }
