...
export const apiRequest = async (
  ...
  if (lastError) {
    ...
    // Only log meaningful errors (not empty or generic)
    // Skip logging if errorMessage is still empty after all attempts
    if (errorMessage && errorMessage.trim() !== '' && errorMessage !== 'Unknown error' && errorMessage !== `Request failed for ${endpoint}`) {
      ...
      // In development, log even if error message is generic to help debug
      console.debug('[API] Request failed silently:', endpoint);
    }
