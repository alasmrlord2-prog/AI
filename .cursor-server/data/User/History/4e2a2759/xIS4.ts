...
URL المحاول: ${url}`;
        console.error('[fetchWithTimeout] Network error details:', {
          url,
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (process.env.NODE_ENV === 'development' || typeof window !== 'undefined') {
        console.group(`[API] Request ${attempt + 1}/${retries + 1}`);
      ...
      try {
        ...
        if (process.env.NODE_ENV === 'development' && typeof window !== 'undefined') {
          console.log(`[API] Attempting to fetch: ${fullUrl}`);
          console.log(`[API] Current window location: ${window.location.href}`);
        ...
        // Log fetch error immediately for debugging with full context
        console.error('[API] ========== FETCH ERROR ==========');
        console.error('[API] URL:', fullUrl);
        console.error('[API] Method:', options.method || 'GET');
        console.error('[API] Headers:', headers);
        console.error('[API] Error object:', fetchError);
        console.error('[API] Error type:', fetchError instanceof Error ? fetchError.constructor.name : typeof fetchError);
        console.error('[API] Error name:', fetchError instanceof Error ? fetchError.name : 'N/A');
