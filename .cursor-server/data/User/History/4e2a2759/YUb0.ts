...
export const getApiUrl = (): string => {
  ...
  // We'll use /api/proxy/* route to proxy requests to backend
  ...
  // Development: Use same origin - Next.js API route will proxy to backend
  // The /api/proxy/* route will handle proxying to the actual backend
  return `${protocol}//${hostname}${port ? `:${port}` : ''}`;
};
...
export const apiRequest = async (
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      // If endpoint starts with /api/, use proxy route in development
      let fullUrl = `${apiUrl}${endpoint}`;
      if (typeof window !== 'undefined' && endpoint.startsWith('/api/')) {
        // In client-side, use proxy route for development
        const hostname = window.location.hostname;
        if (hostname === 'localhost' || hostname === '127.0.0.1') {
          // Replace /api/ with /api/proxy/ to use Next.js API route proxy
          fullUrl = `${apiUrl}/api/proxy${endpoint.replace('/api', '')}`;
        }
      }
