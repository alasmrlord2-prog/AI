...
export default function WorkflowsPage() {
  ...
  const handleExecute = async (workflowId: string) => {
    try {
      ...
      const res = await fetch(`${getApiUrl()}/api/workflows/${workflowId}/execute`, {
        method: "POST",
  ...
  const handleDelete = async (workflowId: string) => {
    ...
    try {
      const res = await fetch(`${getApiUrl()}/api/workflows/${workflowId}`, {
        method: "DELETE"
      });
  ...
  const fetchExecutions = async (workflowId: string) => {
    try {
      const res = await fetch(`${getApiUrl()}/api/workflows/executions?workflow_id=${workflowId}&limit=10`);
      if (res.ok) {
        const data = await res.json();
