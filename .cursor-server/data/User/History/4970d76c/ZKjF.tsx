...
export default function SOCPage() {
  const [activeTab, setActiveTab] = useState<"incidents" | "threat_intel" | "simulation" | "autoresponse" | "investigation" | "malware" | "forensics">("incidents");
  ...
  const handleForensicsAnalysis = async (type: "memory" | "disk", path?: string) => {
    setLoading(true);
    try {
      const token = getAuthToken();
      const endpoint = type === "memory" 
        ? "/api/security/advanced/memory_forensics"
        : "/api/security/advanced/disk_forensics";
      ...
      const res = await fetch(`${API_URL}${endpoint}`, {
        ...
        body: type === "disk" ? JSON.stringify({ path: path || "/" }) : JSON.stringify({}),
      });
