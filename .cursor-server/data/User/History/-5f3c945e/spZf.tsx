...
export default function DigitalTwinPage() {
  ...
  const loadTwins = async () => {
    setLoading(true);
    try {
      const data = await apiRequest("/api/digital-twin", {}, 5000); // 5 second timeout
      setTwins(data.twins || []);
    } catch (error) {
              ...
              <div className="space-y-4">
                ...
                <div className="mt-6">
                  ...
                  <div className="space-y-3">
                    {twins.length > 0 ? (
                      twins.map((twin, idx) => (
                        <Card key={`twin-${twin.twin_id || twin.id || idx}-${idx}`} className="bg-slate-800 border-slate-700">
                      ...
                      <div className="text-slate-500 text-center py-8">No digital twins found</div>
                    )}
                  </div>
                </div>
              </div>
