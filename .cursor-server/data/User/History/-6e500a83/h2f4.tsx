...
export default function BackupPage() {
  ...
  const fetchBackups = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${getApiUrl()}/api/backup/list`);
      if (res.ok) {
  ...
  const handleCreateBackup = async () => {
    try {
      ...
      if (backupType === "postgresql" || backupType === "mysql") {
        endpoint = `${getApiUrl()}/api/backup/${backupType}`;
        body = { db_name: dbName };
      } else if (backupType === "docker-volume") {
        endpoint = `${getApiUrl()}/api/backup/docker-volume`;
  ...
  const handleDeleteBackup = async (backupId: string) => {
    ...
    try {
      const res = await fetch(`${getApiUrl()}/api/backup/${backupId}`, {
        method: "DELETE"
      });
  ...
  const handleVerifyBackup = async (backupId: string) => {
    try {
      const res = await fetch(`${getApiUrl()}/api/backup/verify/${backupId}`);
      if (res.ok) {
        const data = await res.json();
        if (data.match) {
