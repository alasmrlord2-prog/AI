...
export default function DevOpsPage() {
  ...
  const [runningJobs, setRunningJobs] = useState<Pipeline[]>([]);
  const [deployments, setDeployments] = useState<Deployment[]>([]);
  const [shadowDeployments, setShadowDeployments] = useState<Deployment[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      if (tab === "cicd") {
        ...
        setRunningJobs(allPipelines.filter((p: Pipeline) => p.status === "running" || p.status === "pending"));
