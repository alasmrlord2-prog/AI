...
export default function IncidentsPage() {
  ...
  const handleCreateIncident = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(data.detail || "فشل إنشاء Incident");
      }
    } catch (err: any) {
      alert(err.message);
    }
  ...
  const handleResolve = async (incidentId: string) => {
    ...
    try {
      const res = await fetch(`${getApiUrl()}/api/incidents/${incidentId}/resolve`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleUpdateIncident = async (incidentId: string, status: string) => {
    try {
      const res = await fetch(`${getApiUrl()}/api/incidents/${incidentId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
