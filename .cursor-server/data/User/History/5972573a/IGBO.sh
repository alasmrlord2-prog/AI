...
if [ -f "requirements.txt" ]; then
    ...
    $PYTHON_CMD -m pip install -r requirements.txt || {
        echo ""
        echo "⚠️  Some packages failed to install (this is OK if httptools failed)"
        echo "   The application will work without httptools (optional dependency)"
        echo ""
        echo "   If you need httptools, install python3-dev:"
