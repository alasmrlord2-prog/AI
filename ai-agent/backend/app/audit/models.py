...
from sqlalchemy import Column, String, DateTime, ForeignKey, Boolean, Text, JSON, Integer, event
...
def _immutable_guard(mapper, connection, target) -> None:
    if not settings.AUDIT_IMMUTABLE:
        return
    session = object_session(target)
    if session and session.info.get("allow_audit_retention"):
        return
    raise ValueError("Audit logs are immutable and cannot be modified or deleted")


event.listen(AuditLog, "before_update", _immutable_guard)
event.listen(AuditLog, "before_delete", _immutable_guard)
event.listen(LoginLog, "before_update", _immutable_guard)
event.listen(LoginLog, "before_delete", _immutable_guard)
