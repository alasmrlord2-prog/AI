...
# Global storage for scan events (in-memory cache)
_scan_events_cache: List[Dict[str, Any]] = []
_scan_events_lock = threading.Lock()

def add_scan_event(event: Dict[str, Any]):
    """Add a scan event to the SIEM event cache."""
    global _scan_events_cache
    with _scan_events_lock:
        _scan_events_cache.append(event)
        # Keep only last 1000 events to prevent memory issues
        if len(_scan_events_cache) > 1000:
            _scan_events_cache = _scan_events_cache[-1000:]

def get_security_events() -> List[Dict[str, Any]]:
    """Collect security events from various sources"""
    events = []
    
    # Add scan events from cache
    global _scan_events_cache
    with _scan_events_lock:
        # Get events from last 24 hours
        cutoff_time = datetime.now() - timedelta(hours=24)
        recent_scan_events = [
            e for e in _scan_events_cache 
            if datetime.fromisoformat(e.get("timestamp", datetime.now().isoformat())) > cutoff_time
        ]
        events.extend(recent_scan_events)
    
    # Check auth logs (system logs - not using resolve_path as they're system paths)
    try:
    ...
    # Check application logs - use resolve_path for portable paths
    try:
        try:
            app_logs = Path(resolve_path("logs/chat.log"))
        except:
            # Fallback to direct path if resolve fails
            app_logs = Path("logs/chat.log")
...
def get_soc_incidents() -> List[Dict[str, Any]]:
    """Get SOC incidents from security events"""
    events = get_security_events()
    incidents = []
    ...
    incident_map = {}
    for event in events:
        # Create incident key based on type and IP
        ...
        if severity in ["high", "critical"]:
            incident_key = f"{event_type}_{ip}"
            
            if incident_key not in incident_map:
                incident_map[incident_key] = {
                    "id": f"inc-{len(incidents)}",
                    "title": f"{event_type.replace('_', ' ').title()} - {ip}",
                    "severity": severity,
                    "status": "open",
                    "source": event.get("source", "unknown"),
                    "timestamp": event.get("timestamp", datetime.now().isoformat()),
                    "description": event.get("message", ""),
                    "affected_assets": [ip] if ip != "unknown" else [],
                    "event_count": 0,
                    "events": []
                }
            
            incident_map[incident_key]["event_count"] += 1
            incident_map[incident_key]["events"].append(event)
            
            # Update severity if we have critical events
            if severity == "critical":
                incident_map[incident_key]["severity"] = "critical"
    
    # Convert to list and sort by severity and timestamp
    ...
    return incidents[:50]  # Return top 50 incidents
...
def run() -> Dict[str, Any]:
    ...
    try:
        ...
        soc_incidents = get_soc_incidents()
        
        # Calculate threat level based on actual data
        critical_count = metrics.get("critical_severity", 0)
        high_count = metrics.get("high_severity", 0)
        active_threats = metrics.get("active_threats", 0)
        suspicious_ips_count = len(threat_intel.get("suspicious_ips", []))
        
        if critical_count > 0 or active_threats > 10 or suspicious_ips_count > 5:
            threat_level = "critical"
        elif high_count > 5 or active_threats > 5 or suspicious_ips_count > 2:
            threat_level = "high"
        elif high_count > 0 or active_threats > 0:
            threat_level = "medium"
        else:
            threat_level = "low"
        ...
        return {
            ...
            "dashboard": {
                ...
                "active_incidents": len(soc_incidents),
