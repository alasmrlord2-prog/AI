...
from app.identity.models import User, Tenant, TenantUser, UserInvitation, Session, APIToken, Department, Project
...
class IdentityService:
    ...
    def get_tenant_users(
        ...
        """Get users for a tenant (active by default)."""
    ...
    def add_user_to_tenant(
        ...
        if existing:
            if existing.role != role:
                existing.role = role
                existing.permissions_version = (existing.permissions_version or 1) + 1
            else:
                existing.role = role
            existing.status = status
            if not existing.permissions_version:
                existing.permissions_version = 1
            if status == "invited" and not existing.invited_at:
                existing.invited_at = datetime.utcnow()
            if status == "active" and not existing.accepted_at:
        ...
        tenant_user = TenantUser(
            ...
            invited_at=datetime.utcnow() if status == "invited" else None,
            accepted_at=datetime.utcnow() if status == "active" else None
        )
        db.add(tenant_user)
        db.commit()
    ...
    def create_user_invitation(
        ...
        """Create a user invitation token (returns invitation + plain token)."""
        token = secrets.token_urlsafe(32)
        expires_at = datetime.utcnow() + timedelta(days=expires_in_days)

        # Revoke any existing active invitations for this user/tenant
        ...
        token_hash = IdentityService._hash_invitation_token(token)
    ...
    def accept_invitation(
        ...
        if not user:
            raise ValueError("User not found for invitation")

        # Require password if user is not active yet
        if user.status != "active":
            if not password:
                raise ValueError("Password is required to accept this invitation")
            IdentityService._validate_password_policy(password)
            safe_password = password
            if len(safe_password.encode()) > 72:
                safe_password = safe_password.encode()[:72].decode(errors="ignore")
        ...
        # Mark email verified and activate user
        user.email_verified = True
        user.status = "active"
        user.updated_at = datetime.utcnow()
        ...
        if tenant_user:
            ...
            if not tenant_user.accepted_at:
                tenant_user.accepted_at = datetime.utcnow()
        else:
            tenant_user = TenantUser(
    ...
    def _hash_invitation_token(token: str) -> str:
        ...
        if not secret:
            raise ValueError("SECRET_KEY or JWT_SECRET_KEY must be configured for invitation hashing")
