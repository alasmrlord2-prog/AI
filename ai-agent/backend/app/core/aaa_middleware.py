        ...
        try:
            ...
            if not tenant_id:
                AAAMiddleware._set_auth_error(
                    request,
                    "missing_tenant",
                    "Token missing tenant_id - multi-tenant isolation required",
                )
                return None

            # Enforce permissions_version per tenant membership
            permissions_version = payload.get("permissions_version")
            try:
                token_version = int(permissions_version) if permissions_version is not None else 1
            ...
            membership_version = tenant_user.permissions_version or 1
            if token_version < membership_version:
                AAAMiddleware._set_auth_error(
                    request,
                    "permissions_version_mismatch",
                    "Token permissions have been revoked",
                    extra={
                        "user_id": str(user_id),
    ...
    async def account_request(
        ...
        try:
            ...
            if user_context.get("tenant_id"):
                ...
                if subscription:
                    # Emit usage event (event-based accounting)
                    try:
                        subscription_service.SubscriptionService.emit_usage_event(
                            db,
                        ...
                        logger.warning("Failed to emit usage event", exc_info=True)
