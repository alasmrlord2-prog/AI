...
from app.services.auth_login import perform_login, verify_tenant_ticket
import logging
...
async def login(
    ...
    result = perform_login(
        ...
        tenant_id=str(login_data.tenant_id) if login_data.tenant_id else None,
    )
...
@router.post("/select-tenant", response_model=schemas.LoginResponse)
async def select_tenant(
    req: schemas.TenantSelectRequest,
    request: Request,
    db: Session = Depends(get_db),
):
