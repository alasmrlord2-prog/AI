...
export default function LineChart({
  ...
  if (!data || data.length === 0) {
    return (
      <div className="h-48 flex items-center justify-center text-sw-text-muted">
        No data available
      </div>
    );
  }
  ...
  return (
    <div className="w-full">
      {title && <div className="text-sm font-semibold text-sw-text-soft mb-2">{title}</div>}
      <svg width="100%" height={height} viewBox={`0 0 ${chartWidth} ${height}`} className="overflow-visible">
        ...
        {/* Points */}
        {points.map((point, i) => {
          const x = typeof point.x === 'number' && !isNaN(point.x) ? point.x : padding;
          const y = typeof point.y === 'number' && !isNaN(point.y) ? point.y : padding + chartHeight;
          const displayValue = typeof point.value === 'number' && !isNaN(point.value) 
