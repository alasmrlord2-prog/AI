...
export default function SecurityCenterPage() {
  ...
  const [hardening, setHardening] = useState<HardeningSuggestion[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const recommendations = hardeningData?.recommendations as Array<{ title?: string; name?: string; description?: string; message?: string }> | undefined;
        const suggestions = hardeningData?.suggestions || (recommendations ? recommendations.map((r) => ({ title: r.title || r.name, description: r.description || r.message })) : []);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Card className="bg-slate-900 border-slate-800">
              ...
              <CardContent>
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {secrets.length > 0 ? (
                    secrets.filter((s: Secret) => (s as { exposed?: boolean; risk?: string }).exposed || (s as { exposed?: boolean; risk?: string }).risk === "high").slice(0, 10).map((secret, idx) => (
