...
export default function SOCPage() {
  ...
  const loadIncidents = async () => {
    try {
      ...
      if (data) {
        ...
        const incidentCases: IncidentCase[] = (Array.isArray(data.events) ? data.events : []).slice(0, 20).map((event: Record<string, unknown>, idx: number) => {
          const eventObj = event as { type?: string; source?: string; severity?: string; timestamp?: string; message?: string; ip?: string };
          return {
            id: `inc-${idx}`,
            title: `${eventObj.type || "Unknown"} - ${eventObj.source || "Unknown"}`,
            severity: (eventObj.severity?.toLowerCase() || "low") as "critical" | "high" | "medium" | "low",
            status: "open" as "open" | "investigating" | "resolved" | "closed",
            source: eventObj.source || "",
            timestamp: eventObj.timestamp || new Date().toISOString(),
            description: eventObj.message || "",
            affected_assets: eventObj.ip ? [eventObj.ip] : [],
            tags: [eventObj.type || "", eventObj.source || ""],
          };
        });
        ...
        const newAlerts: Alert[] = criticalIncidents.slice(0, 5).map(inc => ({
          id: `alert-${inc.id}`,
          type: inc.severity as "critical" | "high" | "medium" | "low" | "info",
  ...
  const handleThreatIntelligenceCheck = async () => {
    ...
    setLoading(true);
    try {
      const data = await apiRequest("/api/security/advanced/threat_intelligence", {
        method: "POST",
  ...
  const handleRunSimulation = async () => {
    ...
    try {
      ...
      }, 30000); // 30s timeout for simulations
  ...
  const handleMalwareAnalysis = async (filePath: string) => {
    setLoading(true);
    try {
      const data = await apiRequest("/api/security/scan_malware", {
        method: "POST",
        body: JSON.stringify({ path: filePath }),
      }, 30000); // 30s timeout for malware analysis
      if (data) {
  ...
  const handleForensicsAnalysis = async (type: "memory" | "disk" | "network", path?: string) => {
    ...
    try {
      ...
      const data = await apiRequest(endpoint, {
        method: "POST",
        body: JSON.stringify(body),
      }, 30000); // 30s timeout for forensics
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="flex gap-2 mb-6 border-b border-slate-800 overflow-x-auto">
            {[
              ...
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id as "overview" | "incidents" | "alerts" | "investigations" | "simulations" | "threat_intel" | "reporting" | "ml_patterns")}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <Card className="bg-slate-900 border-slate-800">
                  ...
                  <CardContent>
                    <PieChart
                      data={[
                        { label: "Critical", value: incidents.filter(i => i.severity === "critical").length, color: "var(--sw-teal)" },
                        { label: "High", value: incidents.filter(i => i.severity === "high").length, color: "var(--sw-primary-dark)" },
                        { label: "Medium", value: incidents.filter(i => i.severity === "medium").length, color: "var(--sw-secondary-light)" },
                        { label: "Low", value: incidents.filter(i => i.severity === "low").length, color: "var(--sw-blue)" },
                      ].filter(d => d.value > 0)}
                      size={200}
                    />
                  </CardContent>
